...
out_dir <- "/Users/mairahmac/Desktop/RC_TestingNotes/FreshStart_redoAgainAgainAgain_PO_redo/Plotting/mad_2.0/1st50Rounds/all/noCD/prelim_cLogit_Choice_PVSS"
...
dat <- model_df_noCD %>%
  ...
  mutate(
    ...
    PVSS_TotalScore = as.numeric(PVSS_TotalScore),
    PVSS_TotalScore_z = as.numeric(scale(PVSS_TotalScore)),
    PVSS_AvgScore = as.numeric(PVSS_AvgScore),
    PVSS_AvgScore_z = as.numeric(scale(PVSS_AvgScore))
  )
...
mL6_PVSS_clustered <- clogit(
  chosen ~
    points + idealDistance +
    points:PVSS_AvgScore_z + idealDistance:PVSS_AvgScore_z +
    strata(roundID) + cluster(participantID),
  data = dat,
  method = "efron"
)
...
cat("\n\n=================================================\n")
cat("L6: (points + distance) + points:PVSS_AvgScore_z + distance:PVSS_AvgScore_z *participant Cluster\n")
cat("=====================================================\n")
print(summary(mL6_PVSS_clustered))
...
run_participant_diagnostics(
  fit = mL6_PVSS_clustered,
  name = "L6_PVSS_clustered",
  data = dat,
  out_dir = out_dir,
  top_n = 10
)
...
model_list <- list(
  ...
  L6_PVSS_clustered = mL6_PVSS_clustered
)
...
cat("==================================\n")
cat("lambda =", as.numeric(lambda_from(mL6_PVSS_clustered)), "\n")
# -------------------------
...
term_recode <- c(
  ...
  "points:PVSS_AvgScore_z" = "Points × Pt PVSS Score",
  "idealDistance:PVSS_AvgScore_z" = "Distance × Pt PVSS Score"
)
...
save_clogit_bundle(mL6_PVSS_clustered,     "L6_PVSS_clustered",      out_dir, data = dat, term_recode = term_recode)
