# mixedModel_ElapsedTime_AllCoinSets.R
...
out_dir <- "/Users/mairahmac/Desktop/myra_code/Python/RewardCollectors_InputGenerate/analyses/results_R/mixedLm"
...
# 
# m_2 <- lmer(
#   roundElapsed_s ~ coinLabel + coinSet + (1 | participantID),
#   data = model_df,
#   REML = FALSE
# )
# print(summary(m_2))
...
m_3 <- lmer(
  roundElapsed_s ~ coinLabel + TotSesh_runTot_RoundNum + coinSet + (1 | participantID),
  data = model_df,
  REML = FALSE
)
print(summary(m_3))
...
m_5 <- lmer(
  roundElapsed_s ~ coinLabel + recentSwapRate_all + coinSet + (1 | participantID),
  data = model_df,
  REML = FALSE
)
print(summary(m_5))
...
m_6 <- lmer(
  roundElapsed_s ~ coinLabel * TotSesh_runTot_RoundNum + recentSwapRate_all + coinSet + (1 | participantID),
  data = model_df,
  REML = FALSE
)
print(summary(m_6))
...
mL_1 <- lmer(
  roundElapsed_s ~ coinLabel + t_early_20 + t_late_20 + coinSet + (1 | participantID),
  data = model_df,
  REML = FALSE
)
print(summary(mL_1))
...
mL_3 <- lmer(
  roundElapsed_s ~ coinLabel * t_early_20 * t_late_20 + recentSwapRate_all + coinSet + (1 | participantID),
  data = model_df,
  REML = FALSE
)
print(summary(mL_3))
