# mixedModel_LogElapsedTime_AllCoinSets.R
...
out_dir <- "/Users/mairahmac/Desktop/myra_code/Python/RewardCollectors_InputGenerate/analyses/results_R/mixedLm_log"
...
# # -------------------------------------------------------------------
# # Mixed model | M1: Pin Drop Latency (Log Elapsed Time) & Coin Label
# # -------------------------------------------------------------------
# cat("\n\n\n\n\n")
# cat("lmer: log_roundElapsed_s ~ coinLabel\n\n")
# 
# m_1 <- lmer(
#   log_roundElapsed_s ~ coinLabel + (1 | participantID),
#   data = model_df2,
#   REML = FALSE
# )
# print(summary(m_1))
...
# # ----------------------------------------------------------------------------------
# # Mixed model | M2: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label + Coin Layout
# # ----------------------------------------------------------------------------------
# cat("\n\n\n\n\n")
# cat("lmer: log_roundElapsed_s ~ coinLabel + coinSet\n\n")
# 
# m_2 <- lmer(
#   log_roundElapsed_s ~ coinLabel + coinSet + (1 | participantID),
#   data = model_df2,
#   REML = FALSE
# )
# print(summary(m_2))
...
# ------------------------------------------------------------------------------------------------
# Mixed model | M3: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label + Session Time + Coin Layout 
# ------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel + TotSesh_runTot_RoundNum + coinSet\n\n")

m_3 <- lmer(
  log_roundElapsed_s ~ coinLabel + TotSesh_runTot_RoundNum + coinSet + (1 | participantID),
  data = model_df2,
  REML = FALSE
)
print(summary(m_3))
...
# ------------------------------------------------------------------------------------------------
# Mixed model | M4: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label * SessionTime + Coin Layout  
# ------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel * TotSesh_runTot_RoundNum + coinSet\n\n")
...
# -------------------------------------------------------------------------------------------------------------
# Mixed model | M5: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label + Volatility + Coin Layout  
# -------------------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel * TotSesh_runTot_RoundNum + recentSwapRate_all + coinSet\n\n")

m_5 <- lmer(
  log_roundElapsed_s ~ coinLabel + recentSwapRate_all + coinSet + (1 | participantID),
  data = model_df2,
  REML = FALSE
)
print(summary(m_5))
...
# -------------------------------------------------------------------------------------------------------------
# Mixed model | M6: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label * SessionTime + Volatility + Coin Layout  
# -------------------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel * TotSesh_runTot_RoundNum + recentSwapRate_all + coinSet\n\n")

m_6 <- lmer(
  log_roundElapsed_s ~ coinLabel * TotSesh_runTot_RoundNum + recentSwapRate_all + coinSet + (1 | participantID),
  data = model_df2,
  REML = FALSE
)
print(summary(m_6))
...
# --------------------------------------------------------------------------------------------------------------------
# Mixed model | L1: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label + Coin Layout + Early Learning + Late Learning 
# --------------------------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel + t_early_20 + t_late_20 + coinSet\n\n")

mL_1 <- lmer(
  log_roundElapsed_s ~ coinLabel + t_early_20 + t_late_20 + coinSet + (1 | participantID),
  data = model_df2,
  REML = FALSE
)
print(summary(mL_1))
...
# -----------------------------------------------------------------------------------------------------------
# Mixed model | L2: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label + t_early_20 + t_late_20 + Coin Layout
# -----------------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel * t_early_20 * t_late_20 + coinSet\n\n")
...
# -----------------------------------------------------------------------------------------------------------
# Mixed model | L3: Pin Drop Latency (Log Elapsed Time)  ~ Coin Label * t_early_20 * t_late_20 + Coin Layout
# -----------------------------------------------------------------------------------------------------------
cat("\n\n\n\n\n")
cat("lmer: log_roundElapsed_s ~ coinLabel  * t_early_20 * t_late_20 + recentSwapRate_all + coinSet\n\n")

mL_3 <- lmer(
  log_roundElapsed_s ~ coinLabel * t_early_20 * t_late_20 + recentSwapRate_all + coinSet + (1 | participantID),
  data = model_df2,
  REML = FALSE
)
print(summary(mL_3))
