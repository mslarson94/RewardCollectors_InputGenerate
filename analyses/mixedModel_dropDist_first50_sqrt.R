# mixedModel_dropDist_first50_sqrt.R
...
paths <- buildPaths(
  ...
  outDirName = paste0("mixedLm_bothTPs_perf_sqrt_", voi)
)
...
make_formula_sqrt <- function(rhs) {
  as.formula(sprintf("I(sqrt(%s)) ~ %s", voi, rhs))
}
# ----------------------------
# Inputs / outputs
# ----------------------------
...
# -------------------------------------------------------------------------
# Coin-set nonparametric tests
# -------------------------------------------------------------------------

cat("\n\n\n\n\n")
cat("Kruskal-Wallis: dropDist ~ coinSet\n\n")
...
mL_1_formula <- as.formula(
  make_formula_sqrt("t_early_15 + t_late_15 + coinSet + coinLabel + (1 | participantID)"
  )
)
mL_1 <- lmer(
  mL_1_formula,
...
mL_2_formula <- as.formula(
  make_formula_sqrt("t_early_15 * t_late_15 + coinSet + (1 | participantID)"
  )
)
mL_2 <- lmer(
  mL_2_formula,
...
mL_3_formula <- as.formula(
  make_formula_sqrt("t_early_15 + t_late_15 + recentSwapRate_all + coinSet + coinLabel + (1 | participantID)"
  )
)
...
mL_4_formula <- as.formula(
  make_formula_sqrt("t_early_15 + t_late_15 + recentSwapRate_all + coinSet +  orderedCollect + coinLabel + (1 | participantID)"
  )
)
...
mL_5_formula <- as.formula(
  make_formula_sqrt("t_early_15 + t_late_15 + recentSwapRate_all + coinLabel + (1 | participantID)"
  )
)
...
mL_6_formula <- as.formula(
  make_formula_sqrt("t_early_15 + t_late_15 + coinLabel + (1 | participantID)"
  )
)
