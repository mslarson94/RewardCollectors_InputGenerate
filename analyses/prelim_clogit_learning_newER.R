...
# 1) Read data + logging
# -------------------------
dataFile <- "/Users/mairahmac/Desktop/RC_TestingNotes/FreshStart_redoAgainAgainAgain_PO_redo/EventSegmentation/megaFiles_final/mad_2.0/1st50_decisionExpanded_AN.csv"
df <- readr::read_csv(dataFile, show_col_types = FALSE)

out_dir <- "/Users/mairahmac/Desktop/RC_TestingNotes/FreshStart_redoAgainAgainAgain_PO_redo/Plotting/mad_2.0/1st50Rounds/all/noCD/prelim_cLogit_Choice"
...
cat("\n==================================\n")
model_df_noCD <- df %>% filter(!trim_lower(coinSet) %in% c("c", "d"))
# -------------------------
# 2) Basic cleaning
# -------------------------
dat <- model_df_noCD %>%
...
pointCheck <- xtabs(chosen ~ points, data=dat)
print(pointCheck)
...
mL4_learning_clustered <- clogit(
  chosen ~ points + idealDistance +
    points:t_early_15 + idealDistance:t_early_15 +
    points:t_late_15  + idealDistance:t_late_15 +
    strata(roundID) + cluster(sessionID),
  data = dat,
  method = "efron"
)
...
mL6_learning_clustered <- clogit(
  chosen ~
    points + idealDistance +
    # swap-rate moderates sensitivities (identifiable) 
    points:coinSet + idealDistance:coinSet +
    strata(roundID), 
  data = dat,
  method = "efron"
)
...
mL7_learning_clustered <- clogit(
  chosen ~
    points + idealDistance + 
    # swap-rate moderates sensitivities (identifiable) 
    points:main_RR + idealDistance:main_RR +
    strata(roundID), 
  data = dat,
  method = "efron"
)
...
mL8_learning_clustered <- clogit(
  chosen ~
    points + points:coinSet +
    strata(roundID), 
  data = dat,
  method = "efron"
)
...
mL9_learning_clustered <- clogit(
  chosen ~
    points + points:coinSet:t_early_15 + points:coinSet:t_late_15 +
    strata(roundID), 
  data = dat,
  method = "efron"
)

mL10_learning_clustered <- clogit(
  chosen ~
    points + idealDistance +
    # swap-rate moderates sensitivities (identifiable) 
    points:coinSet + idealDistance:coinSet +
...
mL11_learning_clustered <- clogit(
  chosen ~
    points + idealDistance +
    # swap-rate moderates sensitivities (identifiable) 
    points:coinSet + idealDistance:coinSet +
    points:t_early_15:coinSet + idealDistance:t_early_15:coinSet +
    points:t_late_15:coinSet  + idealDistance:t_late_15:coinSet +
    strata(roundID), 
  data = dat,
  method = "efron"
)
...
cat("=====================================================\n")
print(summary(mL7_learning_clustered))

cat("\n\n=================================================\n")
cat("L8: (points + distance * learning)*participant + recentSwapRate_all Cluster\n")
cat("=====================================================\n")
print(summary(mL8_learning_clustered))


cat("\n\n=================================================\n")
cat("L9: (points + distance * learning)*participant + recentSwapRate_all Cluster\n")
cat("=====================================================\n")
print(summary(mL9_learning_clustered))

cat("\n\n=================================================\n")
cat("L10: (points + distance * learning)*participant + recentSwapRate_all Cluster\n")
cat("=====================================================\n")
print(summary(mL10_learning_clustered))


cat("\n\n=================================================\n")
cat("L11: (points + distance * learning)*participant + recentSwapRate_all Cluster\n")
cat("=====================================================\n")
print(summary(mL11_learning_clustered))
...
model_list <- list(
  ...
  L8_learning_clustered = mL8_learning_clustered,
  L9_learning_clustered = mL9_learning_clustered,
  L10_learning_clustered = mL10_learning_clustered,
  L11_learning_clustered = mL11_learning_clustered
)
...
cat("\n\n==============================\n")
cat("Implied lambda from L6\n")
cat("==================================\n")
cat("lambda =", as.numeric(lambda_from(mL6_learning_clustered)), "\n")

cat("\n\n==============================\n")
cat("Implied lambda from L7\n")
cat("==================================\n")
cat("lambda =", as.numeric(lambda_from(mL7_learning_clustered)), "\n")

cat("\n\n==============================\n")
cat("Implied lambda from L10\n")
cat("==================================\n")
cat("lambda =", as.numeric(lambda_from(mL10_learning_clustered)), "\n")

cat("\n\n==============================\n")
cat("Implied lambda from L11\n")
cat("==================================\n")
cat("lambda =", as.numeric(lambda_from(mL11_learning_clustered)), "\n")
...
term_recode <- c(
  ...
  "points:coinSet" = "Points × coin layout",
  "idealDistance:coinSet" = "Distance × coin layout",
  "points:main_RR" = "Points × Main RR Session",
  "idealDistance:main_RR" = "Distance × Main RR Session",
  "points:t_early_15" = "Points × early",
  "idealDistance:t_early_15" = "Distance × early",
  "points:t_late_15" = "Points × late",
  "idealDistance:t_late_15" = "Distance × late",
  "points:recentSwapRate_all" = "Points x volatility",
  "idealDistance:recentSwapRate_all" = "Distance x volatility",
  "points:coinSet:t_early_15" = "Points × coin layout x early",
  "idealDistance:coinSet:t_early_15" = "Distance × coin layout x early",
  "points:coinSet:t_late_15" = "Points × coin layout x late",
  "idealDistance:coinSet:t_late_15" = "Distance × coin layout x late"
)
...
save_clogit_bundle(mL8_learning_clustered,  "L8_Learning_clustered",  out_dir, data = dat, term_recode = term_recode)
save_clogit_bundle(mL9_learning_clustered,  "L9_Learning_clustered",  out_dir, data = dat, term_recode = term_recode)
save_clogit_bundle(mL10_learning_clustered, "L10_Learning_clustered",  out_dir, data = dat, term_recode = term_recode)
save_clogit_bundle(mL11_learning_clustered, "L11_Learning_clustered",  out_dir, data = dat, term_recode = term_recode)
