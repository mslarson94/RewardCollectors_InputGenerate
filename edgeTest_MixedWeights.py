...
graph_preStartList = edgeData_test.iterate_startPos(dataConfigs_TheoPaths.whichWeight, 'Pre')
print('pre is done')
graph_postStartList = edgeData_test.iterate_startPos(dataConfigs_TheoPaths.whichWeight, 'Post')
#print(graph_startList)

allNodeVisitOrders = []

for specGraph in graph_preStartList:
...
print('*'*27)
print('pre is done')
for specGraph in graph_postStartList:
	print('specGraph[0]', specGraph[0])
	newfilename = dataConfigs_TheoPaths.outPath + '/' + specGraph[0] + "_dijkstra.gif"
	nodevisit = edgeData_test.animate_dijkstra(specGraph[1], specGraph[0], newfilename)
	allNodeVisitOrders.append(nodevisit)
