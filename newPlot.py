...
arena_x = [0, 3.5, 5, 3.5, 0, -3.5, -5, -3.5]
arena_y = [5, 3.5, 0, -3.5, -5, -3.5, 0, 3.5]
...
for block_num in blocks:
    ...
    for i, round_num in enumerate(rounds):
        ...
        # Plot the path between waypoints and mark positions just before 'Type' == 'Event'
        for j in range(len(round_data) - 1):
            if round_data.iloc[j + 1]['Type'] == 'Event':
                # Place a red '*' at the position before 'Event'
                ax.plot(round_data['X'].values[j], round_data['Z'].values[j], marker='*', color='red', markersize=10, label='Before Event')
            else:
