...
def safe_parse_details_column(df):
    def clean_and_parse(val):
        if isinstance(val, str) and val.startswith("{") and "'" in val:
            try:
                # Safely evaluate the string back to a dict
...
# def safe_parse_details_column(df):
#     parsed_details = df['details'].apply(parse_details_field)
#     return pd.concat([df.drop(columns=['details']), parsed_details.apply(pd.Series)], axis=1)

# def parse_details_field(details_str):
#     """
#     Converts a 'details' string field into a structured dictionary.
#     Supports fields like:
#     - pinLocal: (x, y, z)
#     - coinPos: (x, y, z)
#     - dropDist, dropQual, coinValue
#     """
#     result = {}
...
#     # Parse all float/int/string fields
...
#             except ValueError:
#                 result[key] = val  # Keep as string if not numeric
...
def extract_summary_data(meta_path, events_path, out_dir="."):
    ...
    # Parse base session date from metadata
    ...
    def fix_time_str(ts):
        if not isinstance(ts, str):
            return pd.NaT
        if ts.count(":") == 3:
            # Replace last ":" with "." to fix milliseconds
            ts = ".".join(ts.rsplit(":", 1))
        try:
    ...
    baseInfo = {
        ...
        "currentRole": "AN"}
    baseMetaInfo = ['file', 'participantID', 'pairID', 'testingDate', 'sessionType', 'ptIsAorB', 'coinSet', 'device', 'main_RR', 'currentRole',]
    # Build summary row
    ...
    # Walking time computation
    print('df head to dict')
    print(df.head(1).to_dict())

    print('startTimestamp type contents to list')
    print(df['start_Timestamp'].dropna().astype(str).head(10).tolist())
    print('startTimestamp head to list')
    print(df['start_Timestamp'].dropna().head(10).tolist())

    print('just start_Timestamp')
    print(df['start_Timestamp'])
    df['start_Timestamp'] = pd.to_datetime(df['start_Timestamp'], errors='coerce')
    df['end_Timestamp'] = pd.to_datetime(df['end_Timestamp'], errors='coerce')

    block = 5  # adjust based on real block numbers
    sub = df[df['BlockNum'] == block]
    print("PinDrops:")
    print(sub[sub['lo_eventType'] == 'PinDrop_Moment'][['chestPin_num','start_Timestamp']])
    print("\nCoinVinEnds:")
    print(sub[sub['lo_eventType'] == 'CoinVis_end'][['chestPin_num','end_Timestamp']])
    ...
    for block_num, group in df.groupby("BlockNum"):
        ...
        if not start_event.empty and not pin_drops.empty:
            walk_time = (pin_drops.iloc[0]["start_Timestamp"] - start_event.iloc[0]["start_Timestamp"]).total_seconds()
...
walk_rows.append({
    ...
    "med_eventType": "Walks"
        ...
        for i in range(1, len(pin_drops)):
            curr_pin_ts = pin_drops.iloc[i]["start_Timestamp"]
            prev_coin = coin_ends[coin_ends["end_Timestamp"] < curr_pin_ts]
            if not prev_coin.empty:
                last_coin = prev_coin.iloc[-1]
                walk_time = (curr_pin_ts - last_coin["end_Timestamp"]).total_seconds()
                walk_rows.append({
                    "BlockNum": block_num,
                    "start_Timestamp": curr_pin_ts,
                    "walk_time_sec": walk_time,
                    "lo_eventType": "Walk_PinDrop",
                    "med_eventType": "Walks"
                })
...
def build_full_walk_rows(events_df, walk_df, output_path):
    ...
    for _, walk in walk_df.iterrows():
        ...
        # Get start event (CoinVis_end with chestPin_num == pin_num)
        ...
        # Get end event (PinDrop_Moment with chestPin_num == pin_num + 1)
        end_event = events_df[
            (events_df["BlockNum"] == block) &
...
build_full_walk_rows(df, walk_df, output_path="./outputs/pinDropWalks_full.csv")
    ...
    # parsed_details_m = marks['details'].apply(parse_details_field)
    # marks_df = pd.concat([marks.drop(columns=['details']), parsed_details_m.apply(pd.Series)], axis=1)

    # parsed_details_pdd = pinDropDist['details'].apply(parse_details_field)
    # pinDropDist_df = pd.concat([pinDropDist.drop(columns=['details']), parsed_details_pdd.apply(pd.Series)], axis=1)

    # parsed_details_sv = swapVotes['details'].apply(parse_details_field)
    # swapVotes_df = pd.concat([swapVotes.drop(columns=['details']), parsed_details_sv.apply(pd.Series)], axis=1)