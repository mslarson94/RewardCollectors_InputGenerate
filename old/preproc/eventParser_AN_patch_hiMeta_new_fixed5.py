...
from eventsParserHelper import (safe_parse_timestamp, backfill_approx_row_indices, build_common_event_fields, 
...
def process_swap_votes_v4a(df, allowed_statuses):
    events = []
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str):
            ...
            if match:
                ...
                try:
                    ...
                    start_ts = start_ts_dt.time().strftime('%H:%M:%S:%f')
                    if start_ts is None:
                    ...
                    events.append({
                        ...
                        "details": {"SwapVote": swapvote, "score": score},
                        "source": "logged",
                        **common_info
                    })
