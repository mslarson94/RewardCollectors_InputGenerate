...
def collapse_meta_files(meta_paths):
    combined = {
        "CoinRegistry": {},
...
def merge_group_files(group_df, base_dir, procDir):
    ...
    final_meta = collapse_meta_files(meta_paths) if meta_paths else None

    return {
        "csv": final_csv,
        "json": final_json,
        "meta": final_meta,
        "group_info": group_df.iloc[0][["participantID", "testingDate", "sessionType", "device"]].to_dict()
    }

def merge_all_version1(base_dir, procDir):
    metadata = load_metadata(base_dir)
    grouped = group_metadata(metadata, ["participantID", "testingDate", "sessionType", "device"])
    merged_data = []

    for _, group_df in grouped:
        merged = merge_group_files(group_df, base_dir, procDir)
        merged_data.append(merged)

    return merged_data


def export_merged_data(merged_data, base_dir, export_dir, group_key_fields):
...
merged_version1_data = merge_all_version1(trueRootDir, procDir)
...
export_merged_data(merged_version1_data, trueRootDir, v1_outDir, v1_groupFields)
