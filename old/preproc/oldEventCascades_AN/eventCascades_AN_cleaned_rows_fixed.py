...
# Removed flatten_event since we retain nested 'details'
    
    
    # keep details dictionary as-is
        # skip flattening
...
def enrich_with_metadata(events, source_file, metadata_df):
    ...
    df.to_json(os.path.join(output_dir, f'{source_file}_events.json'), orient='records', lines=True)
            ...
            while j < len(df):
                ...
                if "Dropped a new pin at" in msg:
                    ...
                    if len(parts) == 5:
                        try:
                            event["details"].update({
                                "drop_quality": parts[0].replace("Dropped a ", ""),
            ...
            if len(parts) == 2:
                try:
                    ...
                    event = {
                        ...
                        "original_row_end": row.Index + SKIP_OFFSET,                        "BlockNum": getattr(row, "BlockNum", None),
        ...
        if isinstance(row.Message, str) and "Chest opened:" in row.Message:
            ...
            if len(parts) == 2 and parts[1].strip().isdigit():
                ...
                event = {
                    ...
                    "original_row_end": row.Index,                    "BlockNum": getattr(row, "BlockNum", None),
                    "RoundNum": getattr(row, "RoundNum", None),
                    "CoinSetID": getattr(row, "CoinSetID", None)
                }
            ...
            if round_code != previous_round:
                ...
                if previous_round in round_event_map and start_idx is not None:
                    events.append({
                        ...
                        "original_row_end": idx - 1 + SKIP_OFFSET,                        "BlockNum": df.at[idx - 1, "BlockNum"],
                ...
                events.append({
                    ...
                    "source": "synthetic",
                    "original_row_start": idx + SKIP_OFFSET,,
                    "original_row_end": idx + SKIP_OFFSET,                    "BlockNum": row.BlockNum,
...
#     output_dir="/Users/mairahmac/Desktop/RC_TestingNotes/SelectedData/Summary"
# )
