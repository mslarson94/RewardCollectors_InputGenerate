...
def common_info_extractor(df, block_status, origRowGrabType, idx=None, j=None, i=None, row=None):
    if origRowGrabType == 'j-1':
        row_start = df.at[idx, "original_index"]
        row_end = df.at[j - 1, "original_index"]
    elif origRowGrabType == 'idx':
        row_start = df.at[idx, "original_index"]
        row_end = row_start
    elif origRowGrabType == 'i':
        row_start = df.at[i, "original_index"]
        row_end = row_start
    elif origRowGrabType == 'row':
        row_start = df.at[row.Index, "original_index"]
        row_end = row_start
    elif origRowGrabType == 'idx-1':
        row_start = df.at[idx - 1, "original_index"]
        row_end = row_start
    else:
        raise ValueError(f"Unknown origRowGrabType: {origRowGrabType}")

    def safe_val(field):
        if row is not None:
            return getattr(row, field, None)
        else:
            return df.at[idx, field]

    return {
        "BlockNum": safe_val("BlockNum"),
        "RoundNum": safe_val("RoundNum"),
        "CoinSetID": safe_val("CoinSetID"),
