...
for file in input_dir.glob("*_pin_drops.csv"):
    ...
    if "BlockNum" not in df.columns or "ParsedTimestamp" not in df.columns or "coinLabel" not in df.columns:
        print('blockNum or parsed timestamp arent in the columns')
        continue
    ...
    for coinSet in df["coinSet"].dropna().unique():
        ...
        for value in ["HV", "LV", "NV"]:
            for qual in ["good", "bad"]:
                filt = (subset["coinLabel"] == value) & (subset["dropQual"] == qual)
                qdata = subset[filt]
                if qdata.empty:
                    continue
