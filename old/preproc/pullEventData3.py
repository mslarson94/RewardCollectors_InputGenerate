...
eventsFileName = "merged_events_R037_AN.csv"
#eventsFileName = "merged_events_R019_PO.csv"
chestsFileName = "ObsReward_A_03_17_2025_14_00_processed_events.csv"
...
eventsDir = os.path.join(baseDir, "MergedEvents_V1/R037_03_17_2025_Afternoon_B_ML2G")
#eventsDir = os.path.join(baseDir, "MergedEvents_V1/R019_03_17_2025_Afternoon_B_ML2C")
procDir = os.path.join(baseDir, "ProcessedData_Flat")

subID = "R037"
#eventsDir = os.path.join(baseDir, "merged_marks")
chestsDir = os.path.join(baseDir, "ExtractedEvents_csv_Flat")
chestsFile = os.path.join(chestsDir, chestsFileName)
eventsFile = os.path.join(eventsDir, eventsFileName)
print(eventsFile)
events_df = pd.read_csv(eventsFile)
chests_df = pd.read_csv(chestsFile)
outDir = os.path.join(baseDir, "onlyMarksAfternoon_AN")
#outDir = os.path.join(baseDir, "onlyMarksAfternoon_PO")
print(outDir)
...
cylinder_types = ["RoundStart", "InterRound_PostCylinderWalk_segment"]
...
trueBlocks.to_csv(os.path.join(outDir, "trueBlocks_1416.csv"))
...
# extractedInfraEvents = extract_epochs_from_merged_events(
#     merged_events_df=infra_events,
#     log_dir=procDir,
#     log_loader=log_loader,
#     padding_seconds=0,
#     timestamp_col='Timestamp'
# )
# extractedInfraEvents.to_csv(os.path.join(outDir, "infrastructureLogExtracts.csv"))


print(len(chests))
extractedChests = extract_epochs_from_merged_events(
    merged_events_df=chests,
    log_dir=procDir,
    log_loader=log_loader,
...
extractedChests.to_csv(os.path.join(outDir, "chests_extracts.csv"))
...
extractedMarks.to_csv(os.path.join(outDir, "onlyMarks_afternoon.csv"))

combined_Cylinderextracts = pd.concat([extractedCylinders, extractedInfras], ignore_index=True)
combined_Chestextracts = pd.concat([extractedInfras, extractedCylinders], ignore_index=True)
combined_Chestextracts.sort_values(by="Timestamp", inplace=True)
combined_Chestextracts.to_csv(os.path.join(outDir, "combined_Chestextracts.csv"), index=False)


combined_Cylinderextracts.sort_values(by="Timestamp", inplace=True)
combined_Cylinderextracts.to_csv(os.path.join(outDir, "combined_Cylinderextracts.csv"), index=False)
# extractedBadDropsEvents = extract_epochs_from_merged_events(
#     merged_events_df=bad_pin_drop_events,
#     log_dir=procDir,
#     log_loader=log_loader,
#     padding_seconds=0,
#     timestamp_col='Timestamp'
# )
# extractedBadDropsEvents.to_csv(os.path.join(outDir, "badDropsLogExtracts.csv"))
