...
root_dir = "/Users/mairahmac/Desktop/RC_TestingNotes/SelectedData/ProcessedData"  # <<< UPDATE THIS TO YOUR ACTUAL ROOT
out_dir = "/Users/mairahmac/Desktop/RC_TestingNotes/SelectedData/Summary"
os.makedirs(out_dir, exist_ok=True)  # Ensure mirrored directory exists
summaryFile = os.path.join(out_dir, "testingSummary_PO.csv")
missingFile = os.path.join(out_dir, "missingFiles_PO.csv")
orphanRows = os.path.join(out_dir, "orphanRows_PO.csv")
orphanRowSummaryFile = os.path.join(out_dir, "orphan_row_summary_PO.csv")
...
valid_files_df = valid_files_df[valid_files_df['primaryRole'] == 'PO']
...
def get_tp2_blocks_by_po_messages(df):
    marker = "A.N. finished a perfect dropround with:"
    msg_df = df[df['Messages_filled'].str.startswith(marker, na=False)]
    block_counts = msg_df.groupby(['BlockNum', 'CoinSetID']).size().reset_index(name='count')

    single_round_blocks = block_counts[(block_counts['count'] == 1) & (block_counts['CoinSetID'].isin([1, 2, 3]))]
    valid_block_nums = single_round_blocks['BlockNum'].unique()

    tp2_entries = df[df['BlockNum'].isin(valid_block_nums) & (df['CoinSetID'].isin([1, 2, 3]))]
    return {
        'TP2_total_blocks': tp2_entries['BlockNum'].nunique(),
        'TP2_Original_blocks': tp2_entries[tp2_entries['CoinSetID'] == 1]['BlockNum'].nunique(),
        'TP2_PPE_blocks': tp2_entries[tp2_entries['CoinSetID'] == 2]['BlockNum'].nunique(),
        'TP2_NPE_blocks': tp2_entries[tp2_entries['CoinSetID'] == 3]['BlockNum'].nunique(),
        'TP2_Validated_BlockNums': tp2_entries['BlockNum'].unique()
    }

def is_perfect_po_message(msg):
    match = re.search(r"A\.N\. finished a perfect dropround with:([0-9.]+)", msg)
    if match:
        return float(match.group(1)) > 0.0
    return False

def count_perfect_po_rounds(df, coinset_id, valid_blocks):
    marker = "A.N. finished a perfect dropround with:"
    ...
    perfects = messages[messages['Messages_filled'].apply(is_perfect_po_message)]
...
for _, row in valid_files_df.iterrows():
    ...
    try:
        ...
        # Skip the first 6 rows BEFORE casting types
        df_usable = df.iloc[6:].copy()
        df_usable['CoinSetID'] = pd.to_numeric(df_usable['CoinSetID'], errors='coerce')
        df_usable['RoundNum'] = pd.to_numeric(df_usable['RoundNum'], errors='coerce')
        df_usable['BlockNum'] = pd.to_numeric(df_usable['BlockNum'], errors='coerce')

        # Orphan detection: last 10 rows only
        ...
        tp2_info = get_tp2_blocks_by_po_messages(df_usable)
        ...
        summaries.append({
            ...
            **tp2_info,
            'TP2_Original_perfect': count_perfect_po_rounds(df_usable, 1, tp2_blocks),
            'TP2_PPE_perfect': count_perfect_po_rounds(df_usable, 2, tp2_blocks),
            'TP2_NPE_perfect': count_perfect_po_rounds(df_usable, 3, tp2_blocks),
        })
...
print("PO summary saved to testingSummary_PO.csv")

if missing_files:
    missing_df = pd.DataFrame(missing_files)
    missing_df.to_csv(missingFile, index=False)
    print(f"Logged {len(missing_files)} missing files to missingFiles_PO.csv")
else:
    print("No missing PO files.")

if orphan_rows:
    orphan_df = pd.concat(orphan_rows)
    orphan_df.to_csv(orphanRows, index=False)
    # Orphan summary per file
    orphan_summary = (
    ...
    print("Wrote orphan_row_summary_PO.csv")
    print(f"Logged {len(orphan_df)} orphaned rows to orphanRows_PO.csv")
else:
