...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the complete global-across-session alignment QC suite "
            "and generate a merged session-level QC summary."
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Root containing *_global_affine_summary.csv and "
            "*_global_affine_mark_diagnostics.csv files."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory containing classify_global_affine_qc.py, "
            "characterize_burst_residuals.py, and "
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Output root for the QC suite. "
            "Defaults to <input-dir>/AlignmentQC."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Recursively search the alignment output directory.",
    )
    ...
    parser.add_argument(
        ...
        default=100.0,
        help=(
            "Descriptive threshold for a large range of stabilized "
            "burst median residuals."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=50.0,
        help=(
            "Descriptive threshold for a large median absolute "
            "consecutive burst shift."
        ),
    )
...
def session_from_global_summary_path(
    value: object,
) -> str:
...
def normalize_session_column(
    ...
    if source == "global_qc":
        if "summary_file" not in out.columns:
            raise KeyError(
                "Global QC output is missing 'summary_file'."
            )

        out["session"] = (
            out["summary_file"]
            .map(session_from_global_summary_path)
        )

    else:
        if "session" not in out.columns:
            raise KeyError(
                f"{source} output is missing 'session'."
            )
...
def build_master_summary(
    ...
    *,
    ...
    global_df = normalize_session_column(
        global_df,
        "global_qc",
    )

    residual_df = normalize_session_column(
        residual_df,
        "burst_residual",
    )

    shift_df = normalize_session_column(
        shift_df,
        "burst_shift",
    )
    ...
    for name, frame in (
        ...
        if duplicated.any():
            ...
            raise ValueError(
                f"{name} contains duplicate sessions: "
                f"{duplicates[:10]}"
            )
    ...
    residual_df = prefix_columns(
        residual_df,
        prefix="burst_residual_",
        keep={"session"},
    )

    shift_df = prefix_columns(
        shift_df,
        prefix="burst_shift_",
        keep={"session"},
    )
    ...
    second_columns = {
        "burst_residual_all_rmse_s":
            "burst_residual_all_rmse_ms",

        "burst_residual_all_mad_s":
            "burst_residual_all_mad_ms",

        "burst_residual_stable_rmse_s":
            "burst_residual_stable_rmse_ms",

        "burst_residual_stable_mad_s":
            "burst_residual_stable_mad_ms",

        "burst_residual_rmse_improvement_excluding_first_s":
            "burst_residual_rmse_improvement_excluding_first_ms",

        "burst_residual_mad_improvement_excluding_first_s":
            "burst_residual_mad_improvement_excluding_first_ms",

        "burst_shift_stable_burst_median_range_s":
            "burst_shift_stable_burst_median_range_ms",

        "burst_shift_median_abs_burst_shift_s":
            "burst_shift_median_abs_burst_shift_ms",

        "burst_shift_max_abs_burst_shift_s":
    ...
    burst_range_ms = safe_numeric(
        master,
        "burst_shift_stable_burst_median_range_ms",
    )

    burst_shift_ms = safe_numeric(
        master,
        "burst_shift_median_abs_burst_shift_ms",
    )
    ...
    master[
        ...
        master["high_burst_median_range"]
        | master["high_median_burst_shift"]
    )

    master[
        "local_structure_available"
    ] = (
        burst_range_ms.notna()
        | burst_shift_ms.notna()
    )
    ...
    all_rmse = safe_numeric(
        master,
        "burst_residual_all_rmse_ms",
    )

    stable_rmse = safe_numeric(
        master,
        "burst_residual_stable_rmse_ms",
    )

    all_mad = safe_numeric(
        master,
        "burst_residual_all_mad_ms",
    )

    stable_mad = safe_numeric(
        master,
        "burst_residual_stable_mad_ms",
    )
...
def main() -> None:
    ...
    if not ok:
        write_stage_report(
            stage_rows,
            out_dir / RUN_REPORT_FILENAME,
        )

        raise RuntimeError(
            "Global affine QC stage failed:\n"
            f"{message}"
        )
    ...
    burst_residual_csv = (
        burst_residual_dir
        / BURST_RESIDUAL_FILENAME
    )

    burst_shift_csv = (
        burst_shift_dir
        / BURST_SHIFT_FILENAME
    )
    ...
    for path in (
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"Expected QC output was not created: "
                f"{path}"
            )
    ...
    master = build_master_summary(
        ...
        high_burst_range_ms=(
            args.high_burst_range_ms
        ),

        high_burst_shift_ms=(
            args.high_burst_shift_ms
        ),
    )
    ...
    # =========================================================
    # Also produce useful category subsets.
    # =========================================================
    ...
    if (
        ...
        for category, group in master.groupby(
            "global_x_local_qc_category",
            dropna=False,
        ):
            safe_category = (
    ...
    # =========================================================
    # Small aggregate category-count table.
    # =========================================================
    ...
    if (
        ...
        category_counts.to_csv(
            out_dir
            / "alignment_qc_category_counts.csv",
            index=False,
        )
    ...
    print()
    print(
        f"[done] QC suite complete"
    )

    print(
        f"[done] sessions in master table: "
        f"{len(master)}"
    )

    print(
        f"[done] master summary -> "
        f"{master_path}"
    )
