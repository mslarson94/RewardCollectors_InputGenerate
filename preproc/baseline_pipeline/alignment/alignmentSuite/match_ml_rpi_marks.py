...
def _parse_args() -> Args:
    ...
    ap.add_argument("--rpi_time_type", default="RPi_Time_simple", help="Exact RPi timestamp column to use.")
...
def main() -> None:
    ...
    rpi_times, rpi_sources, rpi_timezone_offsets = _choose_rpi_times_and_sources(rpi_df,args.rpi_time_type)

    rpi_times = rpi_times.reset_index(drop=True)
    rpi_sources = rpi_sources.reset_index(drop=True)
    rpi_timezone_offsets = rpi_timezone_offsets.reset_index(drop=True)

    if rpi_times.isna().all():
        raise ValueError(f"RPi timestamp column {args.rpi_time_type!r} contains no parseable timestamps")
        ...
        rows.append(
            {
                ...
                "timezone_offset_hours": rpi_timezone_offsets.iloc[j],
                "matcher_initial_match_gap_s": args.initial_match_gap_s,
                "matcher_final_match_gap_s": args.final_match_gap_s,
                "matcher_coarse_search_window_s": args.coarse_search_window_s,
                "matcher_sigma_clip": args.sigma_clip,
    ...
    valid_tz_offsets = (
        rpi_timezone_offsets
        .dropna()
        .unique()
    )

    if len(valid_tz_offsets) == 0:
        summary_tz_offsets = ""
    elif len(valid_tz_offsets) == 1:
        summary_tz_offsets = str(float(valid_tz_offsets[0]))
    else:
        summary_tz_offsets = ";".join(
            str(float(x))
            for x in sorted(valid_tz_offsets)
        )
        ...
        [
            {
                ...
                "timezone_offset_hours": summary_tz_offsets,
