...
from RC_utilities.alignHelpers.batchAlignHelpers import _is_label_col
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default='mLT_orig',
        help="Magic Leap event time column")
    ...
    same_ts = False
    if same_len and (csv_timestamp_column in df_bio.columns and csv_timestamp_column in df_rns.columns):
        same_ts = pd.Series(df_bio[csv_timestamp_column].astype(str).values).equals(
            pd.Series(df_rns[csv_timestamp_column].astype(str).values)
        )

    if not same_len or not same_ts:
        print(f"[warn] BioPac/RNS aligned files differ in length or {csv_timestamp_column}; merging by index.")
