...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch wrapper for read_rpi_logs_to_csv.py. Scans session-level RPi directories from a collated workbook and runs the parser once per session/source directory.")
    ...
    parser.add_argument("--sources", nargs="+", choices=["BioPac", "RNS"], default=["BioPac", "RNS"], help="RPi source folders to process.",)
    parser.add_argument("--output_mode", choices=["flat", "nested"], default="flat", 
        help="flat:  RPi_preproc/{source}/RPi_simple_raw     nested: RPi_preproc/{source}/RPi_simple_raw/{pairID_py}/{testingDate}/{sessionType}")
    parser.add_argument("--summary_csv", default="", help="Optional explicit path for the batch summary CSV.")
    parser.add_argument("--pair_col", default="pairID_py", help="Workbook column holding pair IDs.")
    parser.add_argument("--date_col", default="testingDate", help="Workbook column holding testing dates.")
    parser.add_argument("--session_col", default="sessionType", help="Workbook column holding session types.")
    parser.add_argument("--only_pair_ids", nargs="*", default=None, help="Optional subset of pair IDs.")
    parser.add_argument("--only_testing_dates", nargs="*", default=None, help="Optional subset of testing dates.",)
    parser.add_argument("--only_session_types", nargs="*", default=None, help="Optional subset of session types.")
    parser.add_argument("--dry_run", action="store_true", help="Print planned work without executing subprocesses.")
    parser.add_argument("--stop_on_error", action="store_true", help="Exit on first subprocess failure.")
    parser.add_argument("--check_flat_collisions", action="store_true", help="In flat mode, scan all discovered .log basenames and fail if any duplicate basenames are found.")
...
def load_sessions(
    ...
    if missing:
        raise ValueError(f"Missing required columns in sheet '{sheet_name}': {missing}. Available columns: {list(df.columns)}")
...
def build_log_dir(raw_data_dir: Path, session: SessionKey, source: str) -> Path:
    return  raw_data_dir / session.pair_id_py / session.testing_date / session.session_type / "RPi" / f"{source}_RPi"


def build_out_dir(rpi_preproc_dir: Path, session: SessionKey, source: str, output_mode: str) -> Path:
...
def run_subprocess(python_bin: str, read_script: Path, log_dir: Path, out_dir: Path, dry_run: bool) -> tuple[str, str]:
    command = [
        python_bin,
        str(read_script),
        "--log_dir",
...
def main() -> int:
    ...
    for session in sessions:
        for source in args.sources:
            ...
            print(f"[run] {session.pair_id_py} | {session.testing_date} | {session.session_type} | {source} | {len(log_files)} log(s)")
