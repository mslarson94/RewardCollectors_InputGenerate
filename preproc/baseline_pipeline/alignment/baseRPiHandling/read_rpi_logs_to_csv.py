...
def main() -> None:
    ap = argparse.ArgumentParser(description="Parse all .log files in a directory into per-log CSVs like logFileSample.csv")
    ap.add_argument("--log_dir", required=True, help="Directory containing .log files (non-recursive).")
    ap.add_argument("--out_dir", required=False, default="", help="Output directory for CSVs (default: same as log_dir).")
    args = ap.parse_args()
    ...
    for p in log_paths:
        ...
        df = pd.DataFrame(rows, columns=["RPi_Time_Raw_simple", "DeviceIP", "EnclosingFolder", "LogFileName", "markNumber"],)
