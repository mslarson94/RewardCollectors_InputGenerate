...
# Return the normalized session/base name shared by the pair's related files.
...
# Construct the path of the mark-matches CSV produced by the matching app.
...
# Construct the path of the singles/exclusions CSV produced by the matching app.
...
# Construct the path where this app stores pair-level review votes.
...
# Pick the most appropriate timestamp column available in the RPi CSV, preferring known column
# names.
...
# Add block, ordinal, generated id, stream label, and exclusion metadata needed by the reviewer
# GUI.
...
# Convert common text representations such as true/1/yes into real Python booleans after loading
# CSV data.
...
# Load previously-created match records, add any missing legacy columns, and restore useful
...
# Load mark-level exclusions from the matching stage and normalize their types.
...
# Create one review row per existing match and merge in any previously-saved vote for that
# pair_id.
...
# Copy mark-level exclusions from the singles CSV onto the marks that will be drawn in this pair-
# review GUI.
...
# Assemble everything needed to review one file pair: marks, blocks, existing matches, single
# exclusions, and saved votes.
def load_pair(
    pair: FilePair,
    match_dir: Path,
    review_dir: Path,
...
# Return whether a pair has a non-empty match CSV and is therefore eligible for pair review.
...
# Check whether every active match has already received a valid review vote.
...
class PairVoteReviewApp:
    # Create the pair-review GUI and connect Matplotlib mouse, keyboard, and scroll events to this
    # class's callback methods.
    def __init__(
        self,
        pairs: list[FilePair],
        match_dir: Path,
    ...
    # Return the CSV path where votes for the currently displayed file pair are stored.
    ...
    # Write the current pair-vote table to disk so GUI decisions persist between sessions.
    def save_reviews(
        self,
    ) -> None:
        path = (
    ...
    # Save before navigation, load another file pair's review data, clear selection, and redraw.
    def goto_pair(
        self,
        index: int,
    ) -> None:
    ...
    # Return the time window for either the selected block or the whole session.
    ...
    # Build a combined plotting/hit-testing table for the marks currently inside the selected
    # block.
    def visible_marks(
        self,
    ) -> tuple[
    ...
    # Check whether a given match belongs inside the current block/time view before drawing or
    # hit-testing it.
    def pair_is_visible(
        self,
        row: pd.Series,
    ...
    # Return only the match rows that should participate in the current view.
    def visible_matches(
        self,
    ) -> pd.DataFrame:
        if (
    ...
    # Read the current 1-4 vote associated with a pair_id.
    def vote_for_pair(
        self,
        pair_id: str,
    ) -> Optional[int]:
    ...
    # Apply a review vote to the selected pair, record its label/time, save immediately, and
    # redraw its visual state.
    def set_vote(
        self,
        vote: int,
    ) -> None:
    ...
    # Remove the vote for a selected pair and refresh the display.
    def clear_vote(
        self,
    ) -> None:
        if (
    ...
    # Map a mouse click to the closest displayed match connection, using geometry in screen/plot
    # coordinates rather than requiring an exact click.
    ...
    # Primary mouse callback: ignore toolbar navigation, find the closest pair connection, and
    # make it the selected pair.
    def on_click(
        self,
        event,
    ) -> None:
    ...
    # Move through Events-derived blocks and redraw using the selected block's time bounds.
    def next_block(
        self,
        step: int,
    ) -> None:
    ...
    # Restore the x/y limits recorded by `draw()` so manual pan/zoom can be discarded.
    def reset_zoom(
        self,
    ) -> None:
        if (
    ...
    # Manage status text shown in the figure without storing it in the underlying CSV data.
    def set_status(
        self,
        message: str,
    ) -> None:
    ...
    # Draw the individual mark ticks/points for one stream, including visual treatment for
    # excluded marks.
    def draw_stream(
        self,
        df: pd.DataFrame,
    ...
    # Draw one matched Events↔RPi connection and style it according to exclusion, selection, and
    # review vote.
    ...
    # Summarize how many pair reviews currently fall into each vote category for display in the
    # UI.
    def vote_counts(
        self,
    ) -> dict[int, int]:
    ...
    # Clear and reconstruct the review plot: marks, pair connections, block guides, labels,
    # help/status text, and axis limits.
    def draw(
        self,
...
# Build the session list from command-line inputs and filter it to pairs that have match data to
# review.
