...
def main() -> None:
    ...
    ap.add_argument(
        "--timeCol", default="mLT_orig", help="Magic Leap CSV time column",)
    ...
    if args.timezone_offset_hours.strip().lower() == "auto":
        ...
        if timeCol not in ml_df.columns:
            raise ValueError(f"ML CSV must contain {timeCol} column for auto offset")
        ml_times = pd.to_datetime(ml_df[timeCol], errors="coerce").dropna()
        if ml_times.empty:
