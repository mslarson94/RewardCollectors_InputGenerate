...
using the Events file's mLT_orig directly. Blocks and rounds come from Events.
...
def detect_events_columns(df: pd.DataFrame) -> Dict[str, Optional[str]]:
    ...
    ts_col = cols.get("mlt_orig")  # required for this script
...
# ---------- CLI pipeline ----------
def main() -> None:
    ap = argparse.ArgumentParser(description="Overlay RPi vs Events marks using Events.mLT_orig (datetime).")
    ap.add_argument("--events", required=True, type=Path, help="Events CSV path (must have mLT_orig).")
    ...
    if not ts_col or not et_col:
        raise RuntimeError("Events file must contain 'mLT_orig' and an event-type column (e.g., 'lo_eventType').")
