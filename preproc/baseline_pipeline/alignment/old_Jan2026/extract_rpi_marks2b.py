...
"""
extract_rpi_marks2.py — Extract per-IP mark timestamps by taking the *next non-empty line*
after a line containing "[<device_ip>]" as the timestamp.
...
CLI:
  python extract_rpi_marks2.py \
    --log_file /path/to/log1.log --log_file /path/to/log2.log \
    --session_date 2025-03-17 \
    --device ML2G \
    --device_ip 192.168.50.109 \
    --label RPi \
    --ml_csv_file /path/to/whatever_events_final.csv \
...
from batchAlignHelpers import _read_logfile, _scan_ip_pairs, _normalize_ml_stem, _to_datetime_on_date
...
def main() -> None:
    ...
    df = pd.DataFrame(
        {
            ...
            "Corr_RPi_Timestamp": [r[0] for r in rpi_aligned],
            "Device": args.device,
    ...
    if args.timezone_offset_hours.strip().lower() == "auto":
        ...
        est = _auto_offset_hours(ml_times, pd.to_datetime(df["RPi_Timestamp"], errors="coerce"))
    ...
    df["RPi_Timestamp"] = pd.to_datetime(df["RPi_Timestamp"], errors="coerce")
    # --- Time offset ---
    tz_offset = timedelta(hours=tz_offset_hours)
    df["RPi_Timestamp_Adjusted"] = df["RPi_Timestamp"] + pd.to_timedelta(tz_offset)
    df["Timezone_Offset_Hours"] = tz_offset_hours

    # Naming
    suffixes = [s for s in (args.strip_ml_suffiXes if hasattr(args, "strip_ml_suffiXes") else args.strip_ml_suffixes or "").split(",") if s]
    if args.ml_csv_file:
    ...
    out_dir = out_dir / f"{args.label}" / "RPiMarks"
