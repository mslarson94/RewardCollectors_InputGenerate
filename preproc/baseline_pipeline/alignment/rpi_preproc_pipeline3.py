...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out-dir",
        default="",
        help="Write outputs under <out-dir>/RPi_preproc/... (default: next to each ML CSV)",
    )
    ...
    ap.add_argument(
        "--stage-report-csv",
        default="",
        help="Optional CSV path for per-stage run report",
    )
...
def collect_simple_raw_csvs(ctx: SourceContext, base_dir: Path, proc_dir: str) -> None:
    ...
    for name in ctx.listed_names:
        csv_path = base_dir / proc_dir / "RPi_preproc" / ctx.label / "RPi_simple_raw" / Path(name).with_suffix(".csv")
        if csv_path.exists():
