...
###
"""
you can't use totRoundFrac or roundFrac in general on the adj_walk2Pin event.
It spans over 2 rounds therefore the value in those fields is nonsense.
"""

######## New Coding
colsThatNeedPropagation_wEvents = [
    "round_start_AppTime", "round_start_origRow", "round_end_AppTime", "round_end_origRow", "round_dur_s", "round_index_in_block",
    "block_start_AppTime", "block_start_origRow", "block_end_AppTime", "block_end_origRow", "block_dur_s", "path_order_round",
    "path_step_in_round", "totDistRound", "totDistBlock"
]
...
def input_suffix_for_role(role: str) -> str:
    if role == "AN":
        return "_eventsWalks.csv"
    if role == "PO":
        return "_with_pinDropVoteScore.csv"
    raise ValueError(f"Unsupported role: {role}")


def output_suffix_for_role(role: str) -> str:
    if role == "AN":
        return "_eventsWalks_filled.csv"
    if role == "PO":
        return "_eventsWalks_filled.csv"
    raise ValueError(f"Unsupported role: {role}")
...
def infer_reprocessed_path(events_walks_path: Path, role: str) -> Path:
    """Infer matching *_reprocessed_with_dist.csv in the same directory."""
    name = events_walks_path.name
    in_suffix = input_suffix_for_role(role)

    if not name.endswith(in_suffix):
        raise ValueError(f"Expected *{in_suffix}, got: {name}")

    base = name[: -len(in_suffix)]
    return events_walks_path.with_name(f"{base}_reprocessed_with_dist.csv")
...
def process_events_walks_file(
    events_walks_csv: str,
    role: str,
    reprocessed_csv: str | None = None,
    out_csv: str | None = None,
    debug: bool = True,
) -> str:
    events_path = Path(events_walks_csv)
    reproc_path = Path(reprocessed_csv) if reprocessed_csv else infer_reprocessed_path(events_path, role)
    ...
    if out_csv:
        ...
        in_suffix = input_suffix_for_role(role)
        out_suffix = output_suffix_for_role(role)
        name = events_path.name
        if not name.endswith(in_suffix):
            raise ValueError(f"Expected *{in_suffix}, got: {name}")
        base = name[: -len(in_suffix)]
        out_path = events_path.with_name(f"{base}{out_suffix}")

    dropCols = [
        "HeadPosAnchored_x_start", "HeadPosAnchored_x_end",
        "HeadPosAnchored_y_start", "HeadPosAnchored_y_end",
        "HeadPosAnchored_z_start", "HeadPosAnchored_z_end"
    ]
    ...
    bad_type = df_fixed["lo_eventType"].eq("Adjusted_1st_Walk_PinDrop")
...
def _cli_v1():
    import argparse
    p = argparse.ArgumentParser(description="Fill missing *_start/_end pos/time/dist cols in input CSV using *_reprocessed_with_dist.csv origRow lookups.")
    p.add_argument("--events-walks", required=True, type=str, help="Path to input CSV")
    p.add_argument("--role", type=str, required=True, help="takes the value PO or AN")
    ...
    out_path = process_events_walks_file(
        args.events_walks,
        role=args.role,
        reprocessed_csv=args.reprocessed,
        out_csv=args.out,
        debug=args.debug,
    )
...
def _cli():
    ...
    p = argparse.ArgumentParser(
        description="Fill missing *_start/_end pos/time/dist cols in input CSV using *_reprocessed_with_dist.csv origRow lookups."
    )

    src = p.add_mutually_exclusive_group(required=True)
    src.add_argument("--events-walks", type=str, help="Path to a single input CSV")
    src.add_argument("--events-walks-dir", type=Path, help="Directory containing input CSV files")
    ...
    p.add_argument("--pattern", type=str, default=None, help="(dir mode) Glob pattern")
    ...
    p.add_argument("--in-suffix", type=str, default=None,
    ...
    if args.pattern is None:
        args.pattern = f"*{input_suffix_for_role(args.role)}"

    if args.in_suffix is None:
        args.in_suffix = input_suffix_for_role(args.role)

    if args.out_suffix is None:
        args.out_suffix = output_suffix_for_role(args.role)
