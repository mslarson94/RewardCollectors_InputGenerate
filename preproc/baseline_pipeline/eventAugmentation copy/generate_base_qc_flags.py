...
def add_base_flags_v1(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    require_columns(
        df,
        [
            "roundID",
    ...
    if "isTP2" not in out.columns:
        out["isTP2"] = (
            out["BlockType"].astype("string").str.strip().str.lower().eq("pindropping")
            & pd.to_numeric(out["totalRounds"], errors="coerce").eq(1)
        ).astype("int8")
...
def add_base_flags(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    redundant_flag_columns = [
        column
        for column in out.columns
        if column.startswith("isTP2_round")
        or column.startswith("isTutorial_round")
    ]
    out = out.drop(columns=redundant_flag_columns, errors="ignore")

    if "isTP2" not in out.columns:
        require_columns(out, ["BlockType"])
        out["isTP2"] = (
            out["BlockType"]
            .astype("string")
            .str.strip()
    ...
    out["dropQual_isBad"] = parse_bad_drop(out["dropQual"])
    out["isIncompleteRound_bool"] = parse_boolish(
        out["isIncompleteRound"]
    )
    out["isTutorial"] = (
        pd.to_numeric(
            out["CoinSetID"],
    ...
    round_summary["isPerfectRound"] = (
        round_summary["n_badDrops"] == 0
    ).astype(int)

    round_summary["isEligibleBase"] = (
        ~round_summary["isIncompleteRound"]
        & round_summary["isTutorial"].eq(0)
    )

    round_summary["baseEligibilityReason"] = np.select(
        [
            round_summary["isIncompleteRound"]
            & round_summary["isTutorial"].eq(1),
            round_summary["isIncompleteRound"],
            round_summary["isTutorial"].eq(1),
        ],
        [
...
def main() -> None:
    ...
    metadata = {
        ...
        "notes": {
            ...
            "isTP2": (
                "1 when BlockType is pindropping and totalRounds == 1; "
                "preserved from upstream when already present"),
