#!/usr/bin/env python3
"""
attach_pinDropVote_metrics.py

Attaches PinDropVote_Moment fields from an events file (*_filled_intervalProps.csv)
...
VOTE_TYPE = "PinDropVote_Moment"

VOTE_FIELDS = ["pinDropVote", "pinDropVoteScore"]
...
def build_vote_table(
    ...
    *,
    ...
    """
    events_df = ensure_columns(events_df, VOTE_FIELDS)

    mask = events_df[EVENTTYPE_COL].astype("string").str.strip().eq(VOTE_TYPE)
    sub = events_df.loc[mask].copy()

    cols = nkeys + VOTE_FIELDS
    if sub.empty:
    ...
    *,
    ...
    if swap_tbl.empty:
        ...
        if debug and not quiet:
            print(f"[debug] No PinDropVote_Moment rows found; wrote NA columns to {out_path.name}")
        return
    ...
    if debug and not quiet:
        n_total = len(merged)
        n_have = merged["PinDropVote"].notna().sum()
        print(f"[debug] {interval_path.name}: rows={n_total}, PinDropVote filled={n_have}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Attach PinDropVote fields (PinDropVote_Moment) onto interval rows by BlockNum/BlockInstance/RoundNum.")
