...
def fill_logged_event_positions(events_augmented_file, processed_file, output_file):
    ...
    df_proc['origRow_start'] = df_proc['origRow_start'].astype(int)
    df_proc.set_index('origRow_start', inplace=True)

    # Also make sure event lookup keys are int
    df_events['origRow_start'] = df_events['origRow_start'].astype(int)

    

    # df_proc = df_proc.rename(columns={'original_index': 'origRow_start'})
    # df_proc.set_index('origRow_start', inplace=True)
    ...
    for idx, row in df_events.iterrows():
        if row.get('EventType') == 'logged' and row[position_cols].isnull().any():
            search_index = row['origRow_start'] - 1
