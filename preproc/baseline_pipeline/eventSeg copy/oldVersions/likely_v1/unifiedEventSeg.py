...
from metadata_and_manifest_utils import attach_metadata_to_events, record_to_manifest, save_manifest, pullMetaData, load_filtered_df, get_metadata_row_for_file

from warning_logger import WarningLogger

from eventParser_AN import buildEvents_AN_v4
...
def process_all_obsreward_files(dataDir, metadata, role, segmentType, subDirs=None, allowed_statuses=["complete", "truncated"]):
    ...
    try:
        ...
        if segmentType not in ['barebones', 'full']:
        ...
        for folder in folders_to_process:
            for fname in os.listdir(folder):
                ...
                try:
                    ...
                    enriched_events = pd.DataFrame(enriched_events).sort_values(by=["ParsedTimestamp"])
...
def main():
    ...
    #procDir = 'SmallSelectedData/idealTestFile'
    ...
    process_all_obsreward_files(dataDir, metaDataFile, role='AN', segmentType='barebones')
    process_all_obsreward_files(dataDir, metaDataFile, role='PO', segmentType='barebones')
