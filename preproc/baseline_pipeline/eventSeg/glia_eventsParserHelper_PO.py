...
def process_marks(df, allowed_statuses, role, segmentType):
    ...
    for idx, row in df.iterrows():
        if isinstance(row.Message, str) and "Sending Headset mark" in row.Message:
            common_info = build_common_event_fields_noTime(row, idx, segmentType) ### Fix Me
            appTime = row["AppTime"]
...
def process_true_round_segments(df, allowed_statuses, role, segmentType):
    events = []
    ...
    for idx, row in df.iterrows():
        ...
        if curr_round != prev_round:
            ...
            if prev_round is not None and round_start_idx is not None:
                ...
                events.append(build_segment_event(start_row, start_row, "RoundStart", segmentType))
                print('process_true_round_segments pre build_segment_event on RoundEnd ')
                events.append(build_segment_event(end_row, end_row, "RoundEnd", segmentType)) 
                print("build_segment_event", build_segment_event(start_row, start_row, "RoundStart", segmentType))
            print(events)
            round_start_idx = idx
...
def process_special_round_segments(df, allowed_statuses, role, segmentType):
    """
...
def process_block_segments(df, allowed_statuses, role, segmentType):
    events = []
    df = df.reset_index(drop=True)
...
def process_block_periods_v4(df, allowed_statuses, role, segmentType):
    events = []
    df = df.reset_index(drop=True)
    if role == "PO" and segmentType == "glia":
        timestamp_col = "mLTimestamp_orig"
    ...
    for round_code, (lo_event, hi_meta) in round_event_map.items():
        ...
        common_info = build_common_event_fields_noTime(start_row, start_row.name, segmentType) ### Fix Me
        common_info.update({
            "start_AppTime": appTime,
...
def process_TrueContent(df, allowed_statuses, role, segmentType):
    if role == "PO" and segmentType == "glia":
        timestamp_col = "mLTimestamp_orig"
        start_time = "start_mLT_orig"
        end_time = "end_mLT_orig"
...
def buildGliaEvents_PO_v2(df, role, allowed_statuses, segmentType):
    return (
        process_marks(df, allowed_statuses, role, segmentType) +
        process_true_round_segments(df, allowed_statuses, role, segmentType) +
        process_special_round_segments(df, allowed_statuses, role, segmentType) +
        process_block_segments(df, allowed_statuses, role, segmentType) +
        process_block_periods_v4(df, allowed_statuses, role, segmentType) +
        process_TrueContent(df, allowed_statuses, role, segmentType)
    )