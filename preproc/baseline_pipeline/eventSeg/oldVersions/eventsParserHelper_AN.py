...
def add_elapsed_time_columns_v1(df):
    """
...
def add_elapsed_time_columns(df):
    ...
    Adds SessionElapsedTime, BlockElapsedTime, and RoundElapsedTime columns.
    """
    if 'ParsedTimestamp' not in df.columns:
        print("⚠️ ParsedTimestamp column missing, skipping elapsed time calculations.")
        return df

    # Defensive patch: Ensure 'ParsedTimestamp' is datetime
    if not pd.api.types.is_datetime64_any_dtype(df['ParsedTimestamp']):
