...
final_column_order_AN = ["RPTimestamp", "eRPTimestamp", "RPTs_AN", "eRPTs_AN"] + last_chunk_column_order
final_column_order_PO = ["RPTimestamp_orig", "eRPTimestamp_orig"] + last_chunk_column_order
...
def enhance_timestamp_with_apptime(df, role):
    ...
    """
    if role == "AN":
        timestamps = pd.to_datetime(df["RPTimestamp"], errors="coerce")
    elif role == "PO": 
        timestamps = pd.to_datetime(df["RPTimestamp_orig"], errors="coerce")
    else:
    ...
    if role == "AN": 
        df["eRPTimestamp"] = enhanced_ts
    elif role == "PO":
        df["eRPTimestamp_orig"] = enhanced_ts
    return df
