    ...
    for dirpath, _, filenames in os.walk(baseDir):
        for filename in filenames:
            ...
            if pattern.match(filename):  
                ...
                # Reorder columns
                data['RobustParsedTimestamp'] = data['Timestamp'].apply(lambda ts: robust_parse_timestamp(ts, session_date))
                #data['RobustParsedTimestamp_str'] = data['Timestamp'].apply(lambda ts: robust_parse_timestamp(ts, session_date).strftime("%Y-%m-%d %H:%M:%S.%f"))
