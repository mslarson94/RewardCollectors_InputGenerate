...
def run_steps_01_to_04_for_file(
    *,
    ...
    if _maybe_skip(outs["reprocessed"], skip_existing):
        ...
        df = compute_speed(df, step_col="totDistRound", dt_col="round_dur_s", out_col="avgRoundSpeed")
        df = compute_speed(df, step_col="totDistBlock", dt_col="block_dur_s", out_col="avgBlockSpeed")
    ...
    v = interval_vert["path_order_round"].astype("string").str.replace(" ", "")
    interval_vert["path_order_round_num"] = v.map(PATH_ORDER_MAP)
    h = interval_horz["path_order_round"].astype("string").str.replace(" ", "")
    interval_horz["path_order_round_num"] = h.map(PATH_ORDER_MAP)
    ...
    e = ev_final["path_order_round"].astype("string").str.replace(" ", "")
    ev_final["path_order_round_num"] = e.map(PATH_ORDER_MAP)
