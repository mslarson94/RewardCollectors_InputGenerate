...
def main() -> None:
    ...
    df = pd.DataFrame(
        {
            "RPi_Time_Raw_simple": [r[0] for r in combined_rows],
            "Device": args.device,
            ...
            "markNumber": [(r[3] +1) for r in combined_rows], # Check this: Is the operation r[3] + 1 okay to do? I just want to add 1 to the value of each logPairIndex number to generate the actual markNumber value
        }
    )
    ...
    out_csv = out_dir / f"{base}_{args.label}_RPi_simple.csv"
