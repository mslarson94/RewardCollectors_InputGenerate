...
from RC_utilities.segHelpers.glia_eventsParserHelper_PO import buildGliaEvents_PO
...
def process_all_obsreward_files(dataDir, metadata, role, subDirs=None, allowed_statuses=["complete", "truncated"]):
    ...
    try:
        ...
        nonNestedOutDirs = eventParserFolderCreatePart1(dataDir, "EventSegmentation")
        ...
        for folder in folders_to_process:
            for fname in os.listdir(folder):
                ...
                nestedOutDirs = nestedEventParserDirs(dataDir, output_dir, out_source_file, "EventSegmentation")
                ...
                try:
                    ...
                    timestamp_col = "eMLT_orig" # fix 1/25/2026 
                    ...
                    if role == 'AN':
                        ...
                        glia =  buildGliaEvents_PO(df=df, allowed_statuses=allowed_statuses, role=role)
                        muscles = buildEvents_PO(df, allowed_statuses)
                        all_events = muscles + glia 
                        meta_json = generate_meta_json(df, full_metadata_df, source_file, role)
...
def cli() -> None:
    ...
    parser.add_argument(
        ...
        help="Dataset subdirectory under --trueRootDir that contains 'ProcessedData' or 'ProcessedData_RPi' (e.g., 'FreshStart')."
    )
    parser.add_argument(
