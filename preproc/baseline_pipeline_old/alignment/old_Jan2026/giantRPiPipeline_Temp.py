...
def parse_args() -> argparse.Namespace:
    ...
    # Verb tuning
    ap.add_argument("--marks-timestamp-col", default="Monotonic_Time_Adj_verb",
...
def main() -> None:
    ...
    for _, row in df.iterrows():
        ...
        simple_root = preproc_root / "RPi_simple"
        verb_root = preproc_root / "RPi_verb"
        unified_root = preproc_root / "RPi_unified"
        ...
        for label, fname, subdir in rpi_specs:
            ...
            # --- SIMPLE: extractor -> simple marks into RPi_preproc/RPi_simple/<Label>/RPiMarks
            cmd_simple = [
                "python", str(extract),
                ...
                "--timezone_offset_hours", args.timezone_offset,  # << threaded as requested
            ]
            if args.allow_day_rollover:
                cmd_simple += ["--allow_day_rollover"]
            # place under simple_root (extract keeps <Label>/RPiMarks/<...>_RPiMarks.csv)
            cmd_simple += ["--out_dir", str(simple_root)]
            # append all logs
            for nm in names:
                log_file = Path(args.base_dir) / args.proc_dir / "RawData" / pair / tdate / sess / "RPi" / subdir / nm
                if not log_file.exists():
                    print(f"[skip] {label} log missing: {log_file}")
                    continue
                cmd_simple += ["--log_file", str(log_file)]

            attempted += 1
            if _run(cmd_simple, args.debug, args.dry_run): ok += 1

            # Expected simple marks path (from extractor’s convention)
            simple_marks_csv = simple_root / label / "RPiMarks" / f"{ml_rootname}_{device}_{label}_RPiMarks.csv"
            ...
            verb_full  = verb_root / label / "RPi_VerbFull" / f"{ml_rootname}_{label}_RPi_verb_full.csv"
            verb_short = verb_root / label / "RPi_Verb"     / f"{ml_rootname}_{label}_RPi_verb.csv"
            verb_full.parent.mkdir(parents=True, exist_ok=True)
            verb_short.parent.mkdir(parents=True, exist_ok=True)
            ...
            verb_marks_csv = verb_root / label / "RPiMarks" / f"{ml_rootname}_{device}_{label}_RPi_verb.csv"
            verb_marks_csv.parent.mkdir(parents=True, exist_ok=True)
            ...
            unified_csv = unified_root / label / f"{ml_rootname}_{device}_{label}_RPi_unified.csv"
