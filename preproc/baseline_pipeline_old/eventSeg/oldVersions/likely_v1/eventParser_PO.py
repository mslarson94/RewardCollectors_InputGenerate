...
import traceback
from eventsParserHelper_PO import (safe_parse_timestamp, backfill_approx_row_indices, build_common_event_fields, 
                                generate_synthetic_events_v2, process_marks, process_true_round_segments,
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str):
            ...
            if match:
                ...
                try:
                    ...
                    #atimestamp = start#"AlignedTimestamp": start_row.get("AdjustedTimestamp", None),
                    ...
                    #atimestamp = row.get("AdjustedTimestamp", None)
                    #astart_ts_dt = safe_parse_timestamp(atimestamp)
                    #astart_ts = start_ts_dt.time().strftime('%H:%M:%S:%f') if #astart_ts_dt else None
                    if start_ts is None:
                    ...
                    #synthetic = generate_synthetic_events_v2(start_time, timestamp, atimestamp, offsets_events, common_info, event_meta)
    ...
    while i < len(df):
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str) and "Other participant just dropped a new pin at" in row["Message"]:
            try:
                ...
                timestamp = row["Timestamp"]
                start_ts_dt = safe_parse_timestamp(timestamp)
                start_ts = start_ts_dt.strftime('%H:%M:%S:%f') if start_ts_dt else None
                #atimestamp = row.get("AdjustedTimestamp", None)
                #astart_ts_dt = safe_parse_timestamp(atimestamp)
                #astart_ts = start_ts_dt.time().strftime('%H:%M:%S:%f') if #astart_ts_dt else None
                if start_ts is None:
    ...
    while i < len(df):
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str) and "Other participant just dropped a new pin at" in row["Message"]:
            try:
                ...
                start_ts_dt = safe_parse_timestamp(timestamp)
                start_ts = start_ts_dt.strftime('%H:%M:%S:%f') if start_ts_dt else None
                astart_ts = atimestamp  # This is now optional; adjust if needed
