...
from eventParser_AN_proposed import buildEvents_AN
...
def process_file_list(file_list, metadata, dataDir, allowed_statuses=["complete", "truncated"]):
    ...
    fileNamePattern_AN = re.compile(r"ObsReward_A_\d{2}_\d{2}_\d{4}_\d{2}_\d{2}.*_processed\.csv$")
    fileNamePattern_PO = re.compile(r"ObsReward_B_\d{2}_\d{2}_\d{4}_\d{2}_\d{2}.*_processed\.csv$")
