...
from eventParser_AN import eventBuilder_AN
from eventParser_PO import eventBuilder_PO
        ...
        for fname in os.listdir(folder):
            ...
            try:
                ...
                if role == 'AN':
                    ...
                    all_events = buildEvents_AN(df, allowed_statuses)
                elif role == 'PO':
                    df = load_filtered_df(file_path)
                    os.makedirs(output_dir, exist_ok=True)
...
def process_file_list(file_list, metadata, dataDir, allowed_statuses=["complete", "truncated"]):
    ...
    fileNamePattern_AN = (r"ObsReward_A_\d{2}_\d{2}_\d{4}_\d{2}_\d{2}.*_processed\.csv$")
    fileNamePattern_PO = (r"ObsReward_B_\d{2}_\d{2}_\d{4}_\d{2}_\d{2}.*_processed\.csv$")
    for file_path in file_list:
        try:
            ...
            if fileNamePattern_AN.match(fname):
                output_dir.mkdir(parents=True, exist_ok=True)
                df = load_filtered_df(str(file_path))
                all_events = buildEvents_AN(df, allowed_statuses)
            elif fileNamePattern_PO.match(fname):
                output_dir.mkdir(parents=True, exist_ok=True)
                df = load_filtered_df(str(file_path))
                all_events = buildEvents_PO(df, allowed_statuses)
            else:
                print(f"❓ {fname} doesn't follow the expected file name pattern AND is listed with this pattern in the metadata file")
                print(f"Please correct both the {source_file} of the file and it's value in the metadata file before trying to process it again")
                logger.log(f"❓ {fname} doesn't follow the expected file name pattern AND is listed with this pattern in the metadata file")
                continue
