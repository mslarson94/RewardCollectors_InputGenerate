...
from cascade_windows_utils import find_cascade_windows_from_events, assign_cascade_id, extract_walking_periods_with_cascade_ids
...
def process_pin_drop(df,allowed_statuses):
    ...
    while i < len(df):
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str) and "Just dropped a pin" in row["Message"]:
            #print(df.loc[i, ["AppTime", "Message", "original_index"]])
            cascade_id += 1
            ...
            while j < len(df):
                ...
                if "Dropped a new pin at" in msg:
                    ...
                    if match:
                        try:
                            event["details"].update({
                                "coin_pos_x": float(match.group(1)),
                                "coin_pos_y": float(match.group(2)),
                                "coin_pos_z": float(match.group(3)),
                                "drop_distance": float(match.group(4)),
                                "drop_quality": match.group(5),
                                "coin_value": float(match.group(6)),
                            })
                        except ValueError:
