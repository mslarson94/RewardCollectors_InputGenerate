...
def assign_coin_label(row, registry):
    set_id = str(int(row['CoinSetID']))
    if set_id not in registry:
        return None
    pos = np.array([row['coinPos_x'], row['coinPos_y'], row['coinPos_z']])
    min_dist = float('inf')
    best_label = None
    labels = {0: "LV", 1: "NV", 2: "HV"}
    for i in range(3):
        reg_pos = np.array([
            registry[set_id][str(i)]['deltax'],
            registry[set_id][str(i)]['deltay'],
            registry[set_id][str(i)]['deltaz']
        ])
        dist = np.linalg.norm(pos - reg_pos)
        if dist < min_dist:
            min_dist = dist
            best_label = labels[i]
    return best_label
...
def extract_summary_data(meta_path, events_path, out_dir="."):
    ...
    df['start_mLT'] = df['start_mLT'].apply(fix_time_str)
    df['end_mLT'] = df['end_mLT'].apply(fix_time_str)
    df = df.sort_values('start_mLT').reset_index(drop=True)
    ...
    baseFileInfo = ['BlockNum', 'RoundNum', 'CoinSetID', 'BlockStatus', 'BlockType', 'chestPin_num', 'start_mLT']
    ...
    pinDropDist_df['coinLabel'] = pinDropDist_df.apply(assign_coin_label, axis=1, registry=coin_registry)

    walk_rows = []
    for block_num, group in df.groupby("BlockNum"):
        group = group.sort_values("start_mLT")
        ...
        if not start_event.empty and not pin_drops.empty:
            walk_time = (pin_drops.iloc[0]["start_mLT"] - start_event.iloc[0]["start_mLT"]).total_seconds()
            first_pin = pin_drops.iloc[0]
            walk_rows.append({
                "BlockNum": block_num,
                "start_mLT": first_pin["start_mLT"],
        ...
        for i in range(1, len(pin_drops)):
            curr_pin_ts = pin_drops.iloc[i]["start_mLT"]
            prev_coin = coin_ends[coin_ends["end_mLT"] < curr_pin_ts]
            if not prev_coin.empty:
                last_coin = prev_coin.iloc[-1]
                walk_time = (curr_pin_ts - last_coin["end_mLT"]).total_seconds()
                walk_rows.append({
                    "BlockNum": block_num,
                    "start_mLT": curr_pin_ts,
                    "walk_time_sec": walk_time,
                    "chestPin_num": pin_drops.iloc[i]["chestPin_num"],
                    "RoundNum": pin_drops.iloc[i].get("RoundNum"),
                    "BlockType": pin_drops.iloc[i].get("BlockType"),
                    "lo_eventType": "Walk_PinDrop",
                    "med_eventType": "Walks"
                })
    ...
    for _, walk in walk_df.iterrows():
        ...
        if not start_event.empty and not end_event.empty:
            ...
            combined_row.update({
                ...
                "start_mLT": start["start_mLT"],
                "end_mLT": end["start_mLT"],
...
# Example usage:
df, walk_df = extract_summary_data("ObsReward_A_02_17_2025_15_11_processed_meta.json", "ObsReward_A_02_17_2025_15_11_processed_events.csv", out_dir="./outputs")
