...
def summarize_dataframe(df: pd.DataFrame, file_label: str) -> dict:
    required_columns = [
        ...
        "path_order_round",
        "pinDropVoteScore",
        "pinDropVoteScoreReg_n",
        "pinDropVoteScoreReg_d",
        "pinDropVoteScoreReg",
    ]
    ...
    # Swap Votes
    correct_swap_vote_rounds = int(
        filtered.loc[
            filtered["SwapVoteScore"].astype(str).str.lower() == "correct",
            "round_id",
        ].nunique()
    )
    swap_vote_score = safe_ratio(correct_swap_vote_rounds, tot_rounds)

    # Pin Drop Votes
    correct_pinDrop_vote_rounds = int((filtered["pinDropVoteScore"] == 1).sum())
    
    pinDrop_vote_score = safe_ratio(correct_pinDrop_vote_rounds, tot_rounds)
    ...
    # Pin Drop Votes
    source_file_pinDrop_vote_registered_n = {}
    source_file_pinDrop_vote_registered_d = {}

    for source_file, group in filtered.groupby("source_file", sort=False):
        n_value = get_single_unique_numeric_value(group["pinDropVoteScoreReg_n"])
        d_value = get_single_unique_numeric_value(group["pinDropVoteScoreReg_d"])

        source_file_pinDrop_vote_registered_n[source_file] = None if n_value is None else int(n_value)
        source_file_pinDrop_vote_registered_d[source_file] = None if d_value is None else int(d_value)

    total_pinDrop_vote_registered_n = int(
        sum(v for v in source_file_pinDrop_vote_registered_n.values() if v is not None)
    )
    total_pinDrop_vote_registered_d = int(
        sum(v for v in source_file_pinDrop_vote_registered_d.values() if v is not None)
    )
    total_pinDrop_vote_registered = safe_ratio(
        total_pinDrop_vote_registered_n,
        total_pinDrop_vote_registered_d,
    )
    ...
    summary = {
        ...
        "pinDropVoteScoreReg_n_sum": total_pinDrop_vote_registered_n,
        "pinDropVoteScoreReg_d_sum": total_pinDrop_vote_registered_d,
        "pinDropVoteScoreReg_recalc": total_pinDrop_vote_registered,
        ...
        "sourceFile_pinDropVoteScoreReg_n": json.dumps(source_file_pinDrop_vote_registered_n, ensure_ascii=False),
        "sourceFile_pinDropVoteScoreReg_d": json.dumps(source_file_pinDrop_vote_registered_d, ensure_ascii=False),
