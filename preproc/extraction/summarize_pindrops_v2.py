...
def get_single_unique_value(series: pd.Series):
    """Return one unique non-null value, or all unique values when they differ."""
...
def build_round_id(df: pd.DataFrame) -> pd.Series:
    """Construct a unique round identifier for completed-round aggregation."""
    if (
        "TotSesh_actTest_RoundNum" in df.columns
        and df["TotSesh_actTest_RoundNum"].notna().all()
    ):
    ...
    if "source_file" in df.columns and "RoundNum" in df.columns:
        round_num = (
            pd.to_numeric(df["RoundNum"], errors="coerce")
            .astype("Int64")
            .astype(str)
        )
    ...
    raise ValueError(
        ...
        "Expected either 'TotSesh_actTest_RoundNum' or both "
        "'source_file' and 'RoundNum'."
    )
...
def get_actual_test_rounds(
    ...
    """Return row-level data restricted to the requested actual-test rounds."""
    actual_round = pd.to_numeric(
        working["TotSesh_actTest_RoundNum"],
        errors="coerce",
    )

    actual_test = working.loc[actual_round.notna()].copy()
    actual_test["actual_test_round_num"] = actual_round.loc[
        actual_round.notna()
    ]

    if max_rounds is not None:
        actual_test = actual_test[
            actual_test["actual_test_round_num"] <= max_rounds
        ].copy()
...
def build_actual_test_round_level(
    ...
    return (
        actual_test.sort_values(
            ["actual_test_round_num", "_orig_index"],
            kind="stable",
        )
        .drop_duplicates(
            subset=["actual_test_round_num"],
            keep="last",
        )
        .copy()
    )
...
def summarize_dataframe(
    ...
    """Summarize one participant file, optionally limited to actual-test rounds."""
    required_columns = [
        "CoinSetID",
        "BlockStatus",
        "BlockType",
    ...
    actual_round_level = build_actual_test_round_level(
        actual_test
    )
    ...
    swap_vote_registered_n = int(
        actual_round_level["SwapVote"].notna().sum()
    )
    swap_vote_registered_d = actual_test_rounds_included
    swap_vote_registered_recalc = safe_ratio(
        swap_vote_registered_n,
        swap_vote_registered_d,
    )

    filtered = actual_test[
        (actual_test["CoinSetID_num"] < 4)
        & (
            actual_test["BlockStatus"]
            .astype(str)
            .str.lower()
            == "complete"
        )
        & (
            actual_test["BlockType"]
            .astype(str)
            .str.lower()
            == "pindropping"
        )
    ...
    if filtered.empty:
        raise ValueError(
            "No completed pindropping rows with CoinSetID < 4 "
    ...
    tot_swap_neg = int(
        (swap_type_lower == "neg").sum()
    )
    tot_swap_pos = int(
        (swap_type_lower == "pos").sum()
    )
    ...
    swap_ratio = (
        f"{tot_normal}:{tot_swap_neg}:{tot_swap_pos}"
    )

    tot_rounds = int(
        filtered["round_id"].nunique()
    )

    swap_round_ids = filtered.loc[
        filtered["isSwap_num"] == 1,
        "round_id",
    ].unique()
    ...
    for source_file, group in actual_test.groupby(
        ...
        group_non_null = group[
            group["roundGrandTotal_num"].notna()
        ].sort_values(
            [
    ...
    cumulative_points_rows = actual_test[
        actual_test["roundGrandTotal_num"].notna()
    ].sort_values(
        [
            "actual_test_round_num",
            "_orig_index",
    ...
    avg_round_speed = float(
        round_level["avgRoundSpeed_num"].mean()
    )
    std_round_speed = float(
        round_level["avgRoundSpeed_num"].std()
    )

    avg_path_eff = float(
        round_level["path_eff_raw_num"].mean()
    )
    std_path_eff = float(
        round_level["path_eff_raw_num"].std()
    )

    path_order_counts = (
        round_level["path_order_round"]
    ...
    summary = {
        ...
        "numFiles": num_files,
        "sourceFile_totPoints": json.dumps(
            source_file_tot_points,
            ensure_ascii=False,
        ),
