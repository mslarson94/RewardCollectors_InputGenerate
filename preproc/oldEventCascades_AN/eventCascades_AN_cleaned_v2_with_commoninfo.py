...
def common_info_extractor(df, block_status, origRowGrabType, idx=None, j=None, i=None, row=None):
    ...
    return {
        "BlockNum": row.get("BlockNum") if row is not None else df.at[idx, "BlockNum"],
        "RoundNum": row.get("RoundNum") if row is not None else df.at[idx, "RoundNum"],
        "CoinSetID": row.get("CoinSetID") if row is not None else df.at[idx, "CoinSetID"],
...
def process_pin_drop(df,allowed_statuses):
    ...
    while i < len(df):
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str) and "Just dropped a pin" in row["Message"]:
            ...
            # }
            common_info = common_info_extractor(df, block_status, 'i', i=i)
            event = {
                "AppTime": start_time,
                "Timestamp": timestamp,
