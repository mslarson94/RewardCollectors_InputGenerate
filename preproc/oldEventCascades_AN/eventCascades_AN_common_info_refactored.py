...
def process_pin_drop(df, skip_offset, allowed_statuses):
    events = []
    cascade_id = 0
    i = 0
    ...
    while i < len(df):
        ...
        if row["Type"] == "Event" and isinstance(row["Message"], str) and "Just dropped a pin" in row["Message"]:
            ...
            common_info = {
                ...
                "original_row_start": i + skip_offset,
                "original_row_end": i + skip_offset
            }
...
def process_feedback_collect(df, skip_offset, allowed_statuses):
    events = []
    cached_idvCoinID = {}
            ...
            if len(parts) == 2:
                try:
                    ...
                    event = {
                        ...
                        "details": details,
                        "source": "logged",
                        "original_row_start": row.Index + skip_offset,
                        "original_row_end": row.Index + skip_offset,
...
def process_ie_events(df, skip_offset, allowed_statuses):
    events = []
    for row in df.itertuples():
        block_status = getattr(row, "BlockStatus", "unknown")
        if pd.notna(row.BlockNum) and block_status not in allowed_statuses:
        ...
        if isinstance(row.Message, str) and "Chest opened:" in row.Message:
            ...
            if len(parts) == 2 and parts[1].strip().isdigit():
                ...
                event = {
                    ...
                    "details": details,
                    "source": "logged",
                    "original_row_start": row.Index,
                    "original_row_end": row.Index,
                    "BlockNum": getattr(row, "BlockNum", None),
                    "RoundNum": getattr(row, "RoundNum", None),
                    "CoinSetID": getattr(row, "CoinSetID", None)
                }
...
def process_chest_opened(df, skip_offset, allowed_statuses):
    chest_events = []
    for row in df.itertuples():
...
def process_IE_coin_collected(df, chest_events, skip_offset, allowed_statuses):
    collected_events = []
...
def process_marks(df, skip_offset, allowed_statuses):
    events = []
    for row in df.itertuples():
...
def process_swap_votes(df, skip_offset, allowed_statuses):
    events = []
    for row in df.itertuples():
...
# --- Walking Periods ---
def process_block_periods(df, skip_offset, allowed_statuses):
    events = []
            ...
            if round_code != previous_round:
                ...
                if previous_round in round_event_map and start_idx is not None:
                    events.append({
                        ...
                        "details": {},
                        "source": "synthetic",
                        "original_row_start": idx - 1 + skip_offset,
                        "original_row_end": idx - 1 + skip_offset,
                ...
                events.append({
                    ...
                    "original_row_end": idx + skip_offset,
                    "BlockNum": row.BlockNum,
                    "RoundNum": row.RoundNum,
                    "CoinSetID": row.CoinSetID
                })
...
def extract_walking_periods(df, cascade_events, skip_offset, allowed_statuses):
    seen_rounds = set()
    walking_periods = []
    ...
    for idx, trigger_type in trigger_rows:
        ...
        if end_time is not None:
            walking_periods.append({
                ...
                "source": "synthetic",
                "original_row_start": idx + skip_offset,
                "original_row_end": j - 1 + skip_offset
            })
