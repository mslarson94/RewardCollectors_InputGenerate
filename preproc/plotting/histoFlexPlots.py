...
def plot_block3_roundNum(
    ...
    req = ["BlockNum", variableOfInterest, "RoundNum", "coinLabel", "dropQual", "BlockStatus"]
    ...
    ][["RoundNum", variableOfInterest, "coinLabel", "dropQual", "participantID", "pairID", "coinSet"]].copy()

    fig, ax = plt.subplots(figsize=(10, 6))
    for _, r in block3.iterrows():
        scatter_point(ax, r["RoundNum"], r[variableOfInterest], r["coinLabel"], r["dropQual"])

    title_string = f"{voi_str} in Test Phase 1 (by Round)"
    subtitle_string = _subtitle_from(block3) if "_subtitle_from" in globals() else ""
...
def plot_block3_roundTime(
    ...
    title_string = f"{voi_str} in Test Phase 1 vs Session Elapsed Time"
    subtitle_string = _subtitle_from(block3) if "_subtitle_from" in globals() else ""
    ax.set_title(title_string, fontsize=14, y=1.055)
    if subtitle_string:
        ax.text(0.5, 1.02, subtitle_string, ha="center", fontsize=11, transform=ax.transAxes)

    ax.set_xlabel(f"Session Elapsed Time {voi_UnitStr}")
    ax.set_ylabel(yLabel)
    add_legends(ax)
    fig.tight_layout()
    plt.show()

def plot_blocks_gt3_overall_vs_TP2Blocks_Time(
    df: pd.DataFrame,
    variableOfInterest: str,
    yLabel: str,
    voi_str: str = "Round Elapsed Time",
    ...
    title_string = f"{voi_str} in Test Phase 2 vs Session Elapsed Time"
    subtitle_string = _subtitle_from(gt3) if "_subtitle_from" in globals() else ""
    ax.set_title(title_string, fontsize=14, y=1.055)
    if subtitle_string:
        ax.text(0.5, 1.02, subtitle_string, ha="center", fontsize=11, transform=ax.transAxes)
...
def plot_blocks_gt3_overall_vs_TP2Blocks_BlockNum(
    df: pd.DataFrame,
    variableOfInterest: str,
    yLabel: str,
    voi_str: str = "Round Elapsed Time",
    ...
    gt3 = df[
        ...
        & df["coinLabel"].notna()
        & df["dropQual"].isin(["good", "bad"])
    ][["BlockNum", variableOfInterest, "coinLabel", "dropQual", "participantID", "pairID", "coinSet"]].copy()

    fig, ax = plt.subplots(figsize=(12, 6))
    for _, r in gt3.iterrows():
        scatter_point(ax, r["BlockNum"], r[variableOfInterest], r["coinLabel"], r["dropQual"])

    title_string = f"{voi_str} by Coin Type Across Test Phase 2 Blocks"
    subtitle_string = _subtitle_from(gt3) if "_subtitle_from" in globals() else ""
    ax.set_title(title_string, fontsize=14, y=1.055)
    if subtitle_string:
        ax.text(0.5, 1.02, subtitle_string, ha="center", fontsize=11, transform=ax.transAxes)

    ax.set_xlabel("Test Phase 2 Block #")
    ax.set_ylabel(yLabel)
    add_legends(ax)
    fig.tight_layout()
    plt.show()
...
def plot_hist_kde_by_coin(
    ...
    *,
    ...
    style_x_for_main_and_dots(
        ax, ax_dots,
        main_xlabel=f"{voi_str} {voi_UnitStr}",
