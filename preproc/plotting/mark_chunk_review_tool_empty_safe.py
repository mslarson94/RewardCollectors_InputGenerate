...
def cluster_records_to_dataframe(clusters: list[Cluster]) -> pd.DataFrame:
    columns = [field_info.name for field_info in fields(Cluster)]
    if not clusters:
        return pd.DataFrame(columns=columns)
    return pd.DataFrame(
        [cluster.to_record() for cluster in clusters],
        columns=columns,
    )


def context_records_to_dataframe(contexts: list[ClusterContext]) -> pd.DataFrame:
    columns = [field_info.name for field_info in fields(ClusterContext)]
    if not contexts:
        return pd.DataFrame(columns=columns)
    return pd.DataFrame(
        [context.to_record() for context in contexts],
        columns=columns,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    propose.add_argument("--time-col", default="eMLT_orig", help="Time column.")
