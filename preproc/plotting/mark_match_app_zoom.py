    ...
    for column in df.columns:
        if (
            ...
            and any(
                ...
                for token in ("time", "timestamp", "date", "datetime")
            )
        ):
            return column
...
def build_blocks(
    ...
    blocks["end"] = blocks["end"].where(
        blocks["end"].notna(),
        next_starts,
    )
    blocks["end"] = blocks["end"].fillna(
        ordered[ts_col].max()
    )
...
def assign_block(
    ...
    starts = (
        pd.to_datetime(ordered["start"])
        .values.astype("datetime64[ns]")
    )
    ends = (
        pd.to_datetime(ordered["end"])
        .values.astype("datetime64[ns]")
    )
...
def load_pair(pair: FilePair) -> LoadedPair:
    logger.info(
    ...
    if not ts_col or not event_col:
        raise RuntimeError(
            "Events file requires eMLT_orig and an event-type column: "
            f"{pair.events_file}"
        )
    ...
    if blocks.empty:
        raise RuntimeError(
            f"No blocks could be constructed from {pair.events_file}"
        )

    event_marks_raw = events[
        events[event_col]
        .astype(str)
        .str.lower()
        .eq("mark")
    ...
    logger.debug(
        "RPi time column detected: %r",
        rpi_time_col,
    )

    if not rpi_time_col:
        raise RuntimeError(
            "Could not detect an RPi timestamp column in "
            f"{pair.rpi_file}"
        )
...
def build_pairs_from_directories(
    ...
    for base, events_file in sorted(
        event_map.items()
    ):
        for label in (
            "BioPac",
...
class MarkMatchApp:
    def __init__(
        ...
        if not pairs:
            raise ValueError(
                "At least one Events/RPi pair is required."
            )
        ...
        self.drag_source: Optional[
            tuple[str, str]
        ] = None

        self.selected_entity: Optional[
            tuple[str, str]
        ] = None
        ...
        if "agg" in backend:
            logger.warning(
                "Matplotlib backend is %s. "
                "An Agg backend is non-interactive, "
                "so mouse clicks/drags will not work.",
                matplotlib.get_backend(),
            )
        ...
        logger.info(
            (
                "Matplotlib callbacks connected: "
                "press=%s release=%s key=%s scroll=%s"
            ),
            self.press_connection_id,
            self.release_connection_id,
            self.key_connection_id,
    ...
    def full_session_bounds(
        ...
        return (
            pd.to_datetime(blocks["start"].min()),
            pd.to_datetime(blocks["end"].max()),
        )

    def current_view_bounds(
        self,
    ) -> tuple[pd.Timestamp, pd.Timestamp]:
        ...
        row = self.loaded.blocks[
            self.loaded.blocks["block"]
            .astype(int)
            .eq(block_number)
        ]
        ...
        return (
            pd.to_datetime(block["start"]),
            pd.to_datetime(block["end"]),
        )
    ...
    def next_block(
        ...
        if not blocks:
            logger.warning(
                "No blocks available for block navigation"
            )
            return
        ...
        if str(self.current_block).lower() == "all":
            ...
            try:
                current_index = blocks.index(
                    int(self.current_block)
                )
            except ValueError:
                current_index = 0
    ...
    def _load_saved_state(
        ...
        if single_path.exists():
            ...
            logger.info(
                "Built fresh singles table with %d rows",
                len(self.singles),
            )
    ...
    def find_nearest_mark(
        ...
        if (
            ...
            logger.debug(
                "find_nearest_mark: event outside usable axes"
            )
            return None
        ...
        if marks.empty:
            logger.warning(
                "find_nearest_mark: no visible marks"
            )
            return None
    ...
    def add_match(
        ...
        logger.info(
            "add_match called: source=%r target=%r",
            source,
            target,
        )

        source_stream, source_id = (
            source
        )
        target_stream, target_id = (
            target
        )
        ...
        if (
            ...
            self.set_status(
                "Matches must connect opposite streams."
            )
            return
        ...
        if existing_events_pair:
            ...
            self.set_status(
                f"{events_id} is already "
                f"matched to "
                f"{existing_events_pair}."
            )
            return
        ...
        event_row = self.find_mark(
            "events",
            events_id,
        )

        rpi_row = self.find_mark(
            "rpi",
            rpi_id,
        )
    ...
    def find_nearest_pair(
        ...
        for _, pair in self.matches.iterrows():
            event_time_num = mdates.date2num(
                pd.to_datetime(
                    pair["events_time"]
                )
            )

            rpi_time_num = mdates.date2num(
                pd.to_datetime(
                    pair["rpi_time"]
                )
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Interactive Events ↔ RPi mark matcher."
        )
    )
    ...
    parser.add_argument(
        "--events-dir",
        type=Path,
        help="Events CSV directory.",
    )

    parser.add_argument(
        "--rpi-dir",
        type=Path,
        help="RPi CSV directory.",
    )
...
def main() -> None:
    ...
    logger.info(
        "MarkMatchApp instance retained: %r",
        app,
    )
