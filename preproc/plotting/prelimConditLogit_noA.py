...
dat = dat[dat["coinSet"].astype(str).str.strip().str.lower() != "a"].copy()
...
print('*'*25)
print('')
print('Data in All Participants in All Coin Layouts EXCEPT COINSET A')
print('')
print('*'*25)
print('*'*25)
...
coin_dummies = pd.get_dummies(dat["coinSet"], prefix="coinSet", drop_first=True).astype(float)


print('\n'*3)
print('*'*25)
print('')
print("\nM5: PathType * CoinLayout")
print('ChosenPath ~ PathType + PathType:CoinLayout')
X_alt_layout = pd.concat(
    [dat[["alt"]].astype(float), coin_dummies.mul(dat["alt"], axis=0).add_prefix("a_x_")],
    axis=1
)
r_alt_layout = ConditionalLogit(dat["chosen"], X_alt_layout, groups=dat["roundID"]).fit()
print(r_alt_layout.summary())
print("\nM5 params")
print(r_alt_layout.params)
print('')
print('*'*25)
print('\n'*3)
...
print('*'*25)
print('')
print("\nM6: Utility * CoinLayout")
print('ChosenPath ~ Utility + Utility:CoinLayout')
X_util_layout = pd.concat(
    [dat[["utility"]].astype(float), coin_dummies.mul(dat["utility"], axis=0).add_prefix("u_x_")],
    axis=1
)
r_util_layout = ConditionalLogit(dat["chosen"], X_util_layout, groups=dat["roundID"]).fit()
print(r_util_layout.summary())
print("\nM6 params")
print(r_util_layout.params)
print('')
print('*'*25)
print('\n'*3)
...
print('*'*25)
print('')
print("\nM7: (PathValue + Distance) * CoinLayout")
print('ChosenPath ~ PathValue + Distance + PathValue:CoinLayout + Distance:CoinLayout')
X_vd_layout = dat[["points", "idealDistance"]].astype(float).copy()

for col in coin_dummies.columns:
    X_vd_layout[f"points_x_{col}"] = dat["points"] * coin_dummies[col]
    X_vd_layout[f"dist_x_{col}"] = dat["idealDistance"] * coin_dummies[col]

r_vd_layout = ConditionalLogit(dat["chosen"], X_vd_layout, groups=dat["roundID"]).fit()
print(r_vd_layout.summary())
print("\nM7 params")
print(r_vd_layout.params)
print('')
print('*'*25)
print('\n'*3)
...
for name, res in [
    ...
    ("M4_utility", r4),
    ("M5_altXcoinSet", r_alt_layout),
    ("M6_utilXcoinSet", r_util_layout),
    ("M7_value_distance_XcoinSet", r_vd_layout)
]:
