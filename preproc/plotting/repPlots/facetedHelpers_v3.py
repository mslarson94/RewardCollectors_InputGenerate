...
def _prep_df_for_file(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    if "coinLabel" in df.columns:
...
@dataclass(frozen=True)
class SharedPlotSpec:
    bin_edges: np.ndarray
    xlim: tuple[float, float]
    bin_width: float
    coin_order: tuple[str, ...]


def read_roles_table(path: str | Path) -> pd.DataFrame:
    """Read comma- or tab-delimited role mappings and normalize identifiers."""
    roles = pd.read_csv(path, sep=None, engine="python", dtype="string")
    roles.columns = [str(column).strip() for column in roles.columns]
    required = {"role", "main_seed42", "RR_seed50"}
    missing = sorted(required.difference(roles.columns))
    if missing:
        raise ValueError(f"Roles table is missing columns: {missing}")

    for column in required:
        roles[column] = roles[column].str.strip()

    roles = roles.dropna(subset=list(required))
    duplicate_roles = roles.loc[roles["role"].duplicated(keep=False), "role"].unique().tolist()
    if duplicate_roles:
        raise ValueError(f"Roles table contains duplicate role rows: {duplicate_roles}")
...
def resolve_representative_pair(
    ...
    *,
    ...
    """Resolve exactly one main/RR pair for a role."""
    ...
    matches = roles.loc[roles[role_column].astype("string").str.strip() == str(role).strip()]
    if matches.empty:
        raise ValueError(f"No representative mapping found for role '{role}'.")
    if len(matches) != 1:
        raise ValueError(f"Expected one mapping for role '{role}', found {len(matches)}.")
    ...
    if not main_id or not rr_id or main_id.lower() == "nan" or rr_id.lower() == "nan":
        raise ValueError(f"Role '{role}' has an empty representative session ID.")
...
def validate_representative_pair(
    ...
    *,
    ...
    """Ensure both representatives have usable VOI rows after filtering."""
    if session_column not in df.columns:
        raise ValueError(f"Input data does not contain session column '{session_column}'.")
    if voi not in df.columns:
        raise ValueError(f"Input data does not contain VOI column '{voi}'.")

    session_values = df[session_column].astype("string").str.strip()
    numeric_voi = pd.to_numeric(df[voi], errors="coerce")
    failures: list[str] = []
    for label, session_id in (
        ...
        elif not numeric_voi.loc[selected].notna().any():
            failures.append(f"{label} session '{session_id}' has no numeric '{voi}' rows")
    if failures:
        raise ValueError(f"Invalid {pair.mode} representative pair: " + "; ".join(failures))


def build_shared_plot_spec(
    df: pd.DataFrame,
    *,
    voi: str,
    bin_width: float | None = None,
    ...
    """Compute one immutable bin/axis/category specification for all facets."""
    if voi not in df.columns:
        raise ValueError(f"Input data does not contain VOI column '{voi}'.")
    values = _collect_numeric(df, voi)
    if values.size == 0:
        raise ValueError(f"No numeric values are available for VOI '{voi}'.")

    if xlim is not None:
        lo, hi = sorted((float(xlim[0]), float(xlim[1])))
        if not np.isfinite(lo) or not np.isfinite(hi) or lo == hi:
            raise ValueError("--xlim must contain two distinct finite values.")
        in_range = values[(values >= lo) & (values <= hi)]
        width_source = in_range if in_range.size else values
        width = float(bin_width) if bin_width is not None else _freedman_diaconis_width(width_source)
        if not np.isfinite(width) or width <= 0:
            width = (hi - lo) / max(min_bins, 1)
        n_bins = max(int(np.ceil((hi - lo) / width)), min_bins)
        edges = np.linspace(lo, hi, n_bins + 1)
    else:
        edges = compute_fixed_bin_edges(df, x=voi, bin_width=bin_width, min_bins=min_bins)
        lo, hi = float(edges[0]), float(edges[-1])

    width = float(np.diff(edges).mean())
    if "coinLabel" in df.columns:
        coin_order = tuple(sorted(df["coinLabel"].dropna().astype(str).unique().tolist()))
    else:
        coin_order = tuple()
