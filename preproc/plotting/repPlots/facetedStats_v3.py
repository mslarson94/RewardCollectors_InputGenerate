...
from facetedHelpers_v3 import _opt_mask  # for _enough_for_stats
...
def test_coin_distributions_tp2(
    df: pd.DataFrame,
    *,
    variableOfInterest: str = "truecontent_elapsed_s",
    voi_str: str = "Round Elapsed Time",
    max_totalRounds: int = 1,
    min_n_per_group: int = 10,
    alpha: float = 0.05,
    verbose: bool = True,
):
    req = ["totalRounds", variableOfInterest, "coinLabel", "dropQual"]
    ...
    filt = (
        (sdf["totalRounds"] <= max_totalRounds)
        & sdf["coinLabel"].notna()
...
def test_coin_distributions_all(
    df: pd.DataFrame,
    *,
    variableOfInterest: str = "truecontent_elapsed_s",
    voi_str: str = "Round Elapsed Time",
    ...
    req = [variableOfInterest, "coinLabel", "dropQual"]
    missing = [c for c in req if c not in df.columns]
    if missing:
        raise ValueError(f"Missing columns: {missing}")
    ...
    groups = {k: v[variableOfInterest].to_numpy() for k, v in dat.groupby("coinLabel")}
    groups = {k: x for k, x in groups.items() if len(x) >= min_n_per_group}
    if len(groups) < 2:
        raise ValueError("Need at least two coin types with sufficient data.")
    ...
    if not pair_df.empty:
        ...
        if pair_df["ES_p"].notna().any():
            valid = pair_df["ES_p"].notna()
            es_q = np.full(len(pair_df), np.nan)
            es_q[valid] = _fdr_bh(pair_df.loc[valid, "ES_p"].to_numpy())
            pair_df["ES_q"] = es_q
        else:
            pair_df["ES_q"] = np.nan
    ...
    results = {
        "omnibus": {
            ...
            "ad_p_approx": ad_res.significance_level / 100.0,
            "kw_stat": kw_res.statistic,
            "kw_p": kw_res.pvalue,
        },
        "pairwise": pair_df,
    }
    ...
    if verbose:
        print(f"{voi_str} & Coin Type")
        print("== Omnibus tests ==")
        print(f"Anderson–Darling k-sample: A² = {ad_res.statistic:.3f}, approx p ≈ {ad_res.significance_level / 100.0:.4g}")
        print(f"Kruskal–Wallis: H = {kw_res.statistic:.3f}, p = {kw_res.pvalue:.4g}")
        print()
        print("== Pairwise (Benjamini–Hochberg FDR on KS) ==")
        if pair_df.empty:
            print("No pairwise results.")
        else:
            print(pair_df.to_string(index=False))
            print()
            print("Cliff's δ thresholds (|δ|): small≈0.147, medium≈0.33, large≈0.474")
            print()
            print("Heuristic summary:")
            ad_hit = (ad_res.significance_level / 100.0) <= 0.05
            kw_hit = kw_res.pvalue <= alpha
            print(f"- Omnibus difference detected ({'yes' if (ad_hit or kw_hit) else 'no'}).")
            sig_pairs = int((pair_df["KS_q"] <= alpha).sum()) if "KS_q" in pair_df else 0
            print(f"- Pairwise KS: {sig_pairs}/{len(pair_df)} significant at FDR q≤{alpha}.")
            med_pairs = int((pair_df["Cliffs_delta"].abs() >= 0.33).sum())
            print(f"- {med_pairs} pair(s) show ≥medium effect (|δ|≥0.33).")
