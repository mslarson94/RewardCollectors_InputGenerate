...
def derive_facet_xlim(
    fit: FacetFit,
    *,
    task_progression_column: str,
    explicit_xlim: tuple[float, float] | None,
) -> tuple[float, float]:
    """Derive one facet-specific x range unless explicit limits were supplied."""
    if explicit_xlim is not None:
        return explicit_xlim

    values: list[np.ndarray] = []
    if not fit.facet.data.empty:
        values.append(_finite_values(fit.facet.data[task_progression_column]))
    if fit.predictions is not None:
        values.append(_finite_values(fit.predictions[task_progression_column]))

    combined = (
        np.concatenate([value for value in values if value.size])
        if any(value.size for value in values)
        else np.array([], dtype=float)
    )
    return _padded_limits(combined, padding_fraction=0.02)
...
def create_representative_regression_figure(
    *,
    ...
    if x_axis_mode not in {"shared", "facet"}:
        raise ValueError("x_axis_mode must be one of: shared, facet")

    figure, axes = plt.subplots(
        1,
        3,
        figsize=(18, 6.5),
        sharex=x_axis_mode == "shared" or xlim is not None,
        sharey=True,
    )
    ...
    for index, (axis, fit) in enumerate(zip(axes, fits)):
        panel_xlim = (
            shared_xlim
            if x_axis_mode == "shared" or xlim is not None
            else derive_facet_xlim(
                fit,
                task_progression_column=task_progression_column,
                explicit_xlim=None,
            )
