...
def main() -> None:
    ...
    debug_session = "05_B_main"

    print("\n=== REPRESENTATIVE DEBUG ===")
    print(
        "Raw rows:",
        len(raw.loc[
            raw[args.session_column].astype("string").str.strip().eq(debug_session)
        ]),
    )
    ...
    print(
        "Filtered rows:",
        len(data.loc[
            data[args.session_column].astype("string").str.strip().eq(debug_session)
        ]),
    )
