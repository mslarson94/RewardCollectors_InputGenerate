...
outDir = os.path.join(baseDir, "fullyExtractedEvents_V1")
...
event_types = ["PreBlock_CylinderWalk_segment", "ChestOpenAnimation"]
infra_types = ["Mark", "RoundStart", "InterRound_PostCylinderWalk_segment", "BlockStart", "InterBlock_Idle_segment", "BlockEnd"]
...
extractedChests.to_csv(os.path.join(outDir, "chest_extracts.csv"))
...
combined_extracts = pd.concat([extractedChests, extractedInfras], ignore_index=True)
combined_extracts.sort_values(by="Timestamp", inplace=True)
combined_extracts.to_csv(os.path.join(outDir, "combined_extracts.csv"), index=False)
