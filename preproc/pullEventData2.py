...
outDir = os.path.join(baseDir, "fullyExtractedEvents_V2")
print(outDir)
...
event_types = ["ChestOpenAnimation"]
cylinder_types = ["RoundStart", "InterRound_PostCylinderWalk_segment"]
infra_types = ["Mark", "RoundStart", "BlockStart", "BlockEnd"]
cylinders = get_events_by_lo_eventType(events_df, cylinder_types)
chests = get_events_by_lo_eventType(events_df, event_types)
...
extractedChests.to_csv(os.path.join(outDir, "cylinder_extracts.csv"))
...
extractedCylinders.to_csv(os.path.join(outDir, "cylinder_extracts.csv"))
...
extractedInfras.to_csv(os.path.join(outDir, "infras_extracts.csv"))

combined_Cylinderextracts = pd.concat([extractedChests, extractedInfras], ignore_index=True)
